...
- `python scripts/build_crypto_industry_taxonomy_review_priority.py --source-csv data/sources/crypto_industry_taxonomy_contract_v1/symbol_taxonomy.csv --bars-path data/cache/crypto_usdt_perp_monthly_volume_top50_current_listed_1h_v1/bars_1h.parquet --fetch-coingecko-categories --category-fetch-limit 5`
- `python scripts/build_crypto_industry_taxonomy_review_priority.py --source-csv data/sources/crypto_industry_taxonomy_contract_v1/symbol_taxonomy.csv --bars-path data/cache/crypto_usdt_perp_monthly_volume_top50_current_listed_1h_v1/bars_1h.parquet --review-packet-batch-id 2`
- `python scripts/validate_crypto_industry_taxonomy_review_packet.py --packet-csv research/factor_runs/crypto_top50_factor_library/factor_diagnostics/industry_taxonomy_review_batch_001.csv --source-csv data/sources/crypto_industry_taxonomy_contract_v1/symbol_taxonomy.csv --bars-path data/cache/crypto_usdt_perp_monthly_volume_top50_current_listed_1h_v1/bars_1h.parquet`
- `python scripts/validate_crypto_industry_taxonomy_review_packet.py --packet-csv research/factor_runs/crypto_top50_factor_library/factor_diagnostics/industry_taxonomy_review_batch_002.csv --source-csv data/sources/crypto_industry_taxonomy_contract_v1/symbol_taxonomy.csv --bars-path data/cache/crypto_usdt_perp_monthly_volume_top50_current_listed_1h_v1/bars_1h.parquet --report-stem industry_taxonomy_review_batch_002_validation`
- `python scripts/validate_crypto_industry_taxonomy_review_packet.py --packet-glob 'research/factor_runs/crypto_top50_factor_library/factor_diagnostics/industry_taxonomy_review_batch_*.csv' --source-csv data/sources/crypto_industry_taxonomy_contract_v1/symbol_taxonomy.csv --bars-path data/cache/crypto_usdt_perp_monthly_volume_top50_current_listed_1h_v1/bars_1h.parquet`
- `python scripts/build_crypto_industry_taxonomy_review_validation_rollup.py`
- `python scripts/check_crypto_industry_taxonomy_review_packet_coverage.py --source-csv data/sources/crypto_industry_taxonomy_contract_v1/symbol_taxonomy.csv --diag-dir research/factor_runs/crypto_top50_factor_library/factor_diagnostics`
- `python scripts/build_crypto_industry_taxonomy_review_workbook.py --diag-dir research/factor_runs/crypto_top50_factor_library/factor_diagnostics`
- `python scripts/split_crypto_industry_taxonomy_review_workbook.py --workbook-csv research/factor_runs/crypto_top50_factor_library/factor_diagnostics/industry_taxonomy_review_workbook.csv --output-dir research/factor_runs/crypto_top50_factor_library/factor_diagnostics/industry_taxonomy_reviewed_packets`
- `python scripts/validate_crypto_industry_taxonomy_review_packet.py --packet-glob 'research/factor_runs/crypto_top50_factor_library/factor_diagnostics/industry_taxonomy_reviewed_packets/industry_taxonomy_review_batch_*.csv' --source-csv data/sources/crypto_industry_taxonomy_contract_v1/symbol_taxonomy.csv --bars-path data/cache/crypto_usdt_perp_monthly_volume_top50_current_listed_1h_v1/bars_1h.parquet --out-dir research/factor_runs/crypto_top50_factor_library/factor_diagnostics/industry_taxonomy_reviewed_packets`
- `python scripts/build_crypto_industry_taxonomy_review_validation_rollup.py --diag-dir research/factor_runs/crypto_top50_factor_library/factor_diagnostics/industry_taxonomy_reviewed_packets`
- `python scripts/apply_crypto_industry_taxonomy_review_packet.py --source-csv data/sources/crypto_industry_taxonomy_contract_v1/symbol_taxonomy.csv --packet-glob 'research/factor_runs/crypto_top50_factor_library/factor_diagnostics/industry_taxonomy_reviewed_packets/industry_taxonomy_review_batch_*.csv' --bars-path data/cache/crypto_usdt_perp_monthly_volume_top50_current_listed_1h_v1/bars_1h.parquet --output-csv /tmp/symbol_taxonomy.reviewed.csv`
- `python scripts/apply_crypto_industry_taxonomy_review_packet.py --source-csv data/sources/crypto_industry_taxonomy_contract_v1/symbol_taxonomy.csv --packet-csv research/factor_runs/crypto_top50_factor_library/factor_diagnostics/industry_taxonomy_review_batch_001.csv --bars-path data/cache/crypto_usdt_perp_monthly_volume_top50_current_listed_1h_v1/bars_1h.parquet --output-csv /tmp/symbol_taxonomy.reviewed.csv`
- `python scripts/apply_crypto_industry_taxonomy_review_packet.py --source-csv data/sources/crypto_industry_taxonomy_contract_v1/symbol_taxonomy.csv --packet-csv research/factor_runs/crypto_top50_factor_library/factor_diagnostics/industry_taxonomy_review_batch_001.csv --packet-csv research/factor_runs/crypto_top50_factor_library/factor_diagnostics/industry_taxonomy_review_batch_002.csv --bars-path data/cache/crypto_usdt_perp_monthly_volume_top50_current_listed_1h_v1/bars_1h.parquet --output-csv /tmp/symbol_taxonomy.reviewed.csv`
- `python scripts/apply_crypto_industry_taxonomy_review_packet.py --source-csv data/sources/crypto_industry_taxonomy_contract_v1/symbol_taxonomy.csv --packet-glob 'research/factor_runs/crypto_top50_factor_library/factor_diagnostics/industry_taxonomy_review_batch_*.csv' --bars-path data/cache/crypto_usdt_perp_monthly_volume_top50_current_listed_1h_v1/bars_1h.parquet --output-csv /tmp/symbol_taxonomy.reviewed.csv`
- `python scripts/check_crypto_industry_taxonomy_review_source.py --source-csv data/sources/crypto_industry_taxonomy_contract_v1/symbol_taxonomy.csv --bars-path data/cache/crypto_usdt_perp_monthly_volume_top50_current_listed_1h_v1/bars_1h.parquet`
...
`build_crypto_industry_taxonomy_review_priority.py` 只按 bars 里的 quote_volume 排人工审核优先级，并可附带 CoinGecko 映射证据、CoinGecko category 审核证据、blocked Alpha101 factor IDs、已人工标记 `OK` 行的 point-in-time 覆盖率预览、`known_at` 是否早于当前评估 bars 的时间对齐检查、人工审核 batch plan 和当前 batch packet；它不推断或填充 sector/industry/subindustry，也不会把 REVIEW 行改成 OK。`validate_crypto_industry_taxonomy_review_packet.py` 是 apply 前的审核包 gate：默认要求至少一个 `target_quality_flag == OK` 行，并检查 target group/timing 字段、重复 symbol、source symbol 存在性和 latest-bar 时间对齐；空白模板结构预检才使用 `--allow-no-ok`。多批并行审核时优先用 `--packet-glob`，它会自动为每个 `industry_taxonomy_review_batch_<NNN>.csv` 写出 `industry_taxonomy_review_batch_<NNN>_validation` 报告；单批手动复核才用 `--report-stem` 保留指定 packet 的独立验证报告。`build_crypto_industry_taxonomy_review_validation_rollup.py` 汇总指定目录下所有 batch-specific validation 报告，显示哪些 batch 已可 apply、哪些仍 blocked；原始空白 packet 目录和人工审核后的 `industry_taxonomy_reviewed_packets/` 目录都要分别 rollup。`check_crypto_industry_taxonomy_review_packet_coverage.py` 检查 packet 集合是否覆盖 source symbols、batch id 是否连续、packet 内/跨 packet 是否重复；它不要求人工 OK 行，也不应用分类。`build_crypto_industry_taxonomy_review_workbook.py` 只把 batch packets 合并成一个人工审核 CSV，方便填 `target_*` 字段；它不替代 validation/apply gate。人工填完 workbook 后，用 `split_crypto_industry_taxonomy_review_workbook.py` 拆回 reviewed batch packets，再对 reviewed packet 目录运行 validate、rollup、apply；拆分工具不验证 OK 行、不应用分类、不覆盖原始空白 packets。`check_public_factor_integration_status.py` 会把当前 packet validation pass/blocker、approved rows、原始 batch rollup、reviewed batch rollup 和 coverage share 汇总到 `taxonomy_readiness`。`apply_crypto_industry_taxonomy_review_packet.py` 只复制人工明确填好的 `target_*` 字段；人工审核多批后可重复传 `--packet-csv` 或用 `--packet-glob` 按 batch id 顺序应用多个 packet，仍然只应用 `target_quality_flag == OK` 行，并用 `--bars-path` 拒绝晚于当前评估 bars 的 known/effective timestamps；同一 symbol 只能在一个 packet 中被标记为 OK，跨 packet 重复 OK 会被 apply 拒绝。source 和 artifact contract 都要求 `effective_from <= known_at`。建议先输出到临时 CSV 并通过 source checker 后再替换源文件。正式解锁仍必须通过 artifact、contract 和 coverage gate。
