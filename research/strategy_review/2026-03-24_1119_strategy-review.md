...
- 当前 runtime state 显示：
  ...
  - 最新 admission 证据是 `research/optimization_loop/2026-03-24_1046_crypto-stat-arb-p2-honesty-execution.md`
...
1. `2026-03-24_1117_strategy-review.md`
   - 正确把主资源继续放在 `Rank 154` 的 `P2 admission close-out`。
2. `2026-03-24_1024_strategy-review.md`
   - 正确把 admission 收口点压在 `effectiveness / cross-asset / parameter`，而不是回头重开旧对象。

结论：当前排班连续性正常；09:22 fresh intake，09:50 唯一 follow-up 升 `P2`，10:18 与 10:46 连续补 time + honesty，因此现在唯一该收口的是 `P2 admission`，不是回头拉旧候选，也不是提前切回 fresh intake。
...
1. `Active P2 slot（Rank 154 / Crypto-Stat-Arb）`：做最小 P2 admission close-out，优先围绕 `effectiveness / cross-asset / parameter` 收口，并直接判断当前证据更接近 `promote_P3` 还是 `drop_to_background`。
...
- 新增：`/root/clawd/jerry/momentum/research/strategy_review/2026-03-24_1119_strategy-review.md`
