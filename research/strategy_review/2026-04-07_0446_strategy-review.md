# 2026-04-07 04:46 UTC strategy review
...
- 数据口径公开可取，最小状态机已成形
- 唯一高杠杆问题已被压清：`PB14-L / PB12 / FLIQ-L` 中是否至少有一个分支在诚实 `fee / slippage / funding` 口径下仍保留可迁移净边

这符合 policy 对 survivor 唯一一次 follow-up 的要求：做一次最小 decisive check，然后直接决定 `promote_P2` 或 `background / P0`。
...
- 最近 `research/optimization_loop/` 的新增 evidence 仍只改变两件事：
...
- 最近 `research/strategy_review/2026-04-07_0429_strategy-review.md` 与 `2026-04-07_0437_strategy-review.md` 的判断，与当前 runtime 一致；到本轮为止未出现新的 runtime-level 证据要求重排或升降级。
...
## 发布与通知
- 中文邮件已发送：主题 `[momentum-bot2-review] 前排不变，继续先收口Rank354`。
- 首页发布脚本已执行到 `build_site_index.py` 之前的流程，但脚本内含 `sudo mkdir/install/chown`；当前 cron 运行态不提供可用 elevated 权限，普通执行会卡在 `sudo`，显式提权执行也被 runtime 直接拒绝，因此本轮首页未成功刷新。
