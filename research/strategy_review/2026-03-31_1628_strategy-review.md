# 2026-03-31 16:28 UTC strategy review
...
2. **本轮 `fresh intake` 是什么？**
   - 结论：**`cointegration pair + graduation + daily throttle`（`Rank 269`）。**
   - 证据：`Fresh intake slot.current_target` 仍是该对象，且 `latest_result_record = research/optimization_loop/2026-03-31_1532_rank269_cointegration_graduation_daily_throttle_intake_keep_p1.md` 已明确写成 fresh intake 首判完成、正式分配 `Rank 269` 并首判 `keep_P1`。

3. **上一条 fresh intake 是否值得那唯一一次 follow-up？**
   - 结论：**值得。**
   - 证据：`Surviving candidate slot.current_target = Rank 269`，`followup_budget_remaining = 1`；最近结论明确说明它保留下来的关键不是作者自报结果，而是 `cointegration spread MR + recent expectancy graduation + daily throttle` 这套可独立审计的 pairs operating skeleton。最便宜且 decisive 的下一步，就是统一成本口径下的小 pairbook clean-room replication，判断它是否真的能迁移到 desk 环境。
...
- `Surviving candidate slot.current_target = Rank 269`
- `Active P2 slot.current_target = none`
- 当前前排对象里唯一占槽的是 `Rank 269`，其正式 rank 已存在。
- 结论：**本轮无需补 rank。**
...
- 因此本轮不应伪造 `P3`，而应优先收口 `Rank 269` 的 survivor follow-up。
...
3. `P1 survivor follow-up`：有，且必须优先执行 `Rank 269` 的唯一一次 decisive follow-up；
4. 在 survivor 动作已经被诚实排到前部后，才用剩余预算补新的 `fresh intake`。

结合最近新 digest，本轮把 `cycle_plan` 重写为：
1. `Rank 269 / cointegration pair + graduation + daily throttle` survivor follow-up
2. `front/back annualized basis calendar spread`
3. `stablecoin discount → peer-parity reversion`
4. `rolling-VWAP anchor × short-rich basket`

这样写符合 policy：
- 没把新的 intake 排到现存 `P1 survivor` 前面；
- 前两项都是真实推进动作，不是空模板；
- 新 intake 都来自最近新的 digest，而不是从 background pool 自动 reopen 旧候选；
- 没继续沿 `Rank 267` 的旧 axis 低杠杆重复。
...
- 更新内容：
  ...
  - 保持 `Rank 269` 作为当前 survivor；
  - 仅重写当前轮 `cycle_plan`，把 survivor follow-up 放回第 1 位，并补入 3 条具体 fresh intake；
