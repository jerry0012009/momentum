# 2026-04-11 03:06 UTC · Rank 11 park reframe

## Selected rank
- `Rank 11`
- selection note: 本轮按 `Rank 1~37` 的 parked rank 低频轮转处理，优先避开最近 7 天内刚复盘过的条目。`Rank 11` 上次 bot6 复盘是 `2026-04-03 11:19 UTC`，已超过 7 天；同时它仍属于 `Rank 1~24` 中**尚无已落地 Rank 11b** 的旧 parked rank，符合 brief 里“优先看尚未有过 `Rank Nb` 尝试的”对象。

## Original park reason
原始 authoritative 证据：
- `research/optimization_loop/2026-03-16_2343_rank11-clean-replication-park.md`
- `research/park_reframe/2026-04-03_1119_rank11-park-reframe.md`
...
翻成人话：
- 原问题不是“模式识别大致对，只差一层确认”；
- 而是这条 `causal extrema` pattern gate 自身就没有形成够厚、够稳、够可迁移的策略主体；
- 因此原 `park` 的审计意义必须保留：失败对象是 **`Rank 11` 这套 pattern gate 值不值得继续占 queue-facing 预算**，而不是所有 reversal / event 主题都死了。
...
2. 最近可用旁证没有把原 trigger 本体变厚，只是继续说明“pattern / event”这个大主题若要活，应该换成新的、更窄、更像 raw alpha 的宿主；
...
2. 但更诚实的写法，是 `CUSUM 事件条 + Triple Barrier` 这种完整 event-driven raw alpha，或 `大实体 engulfing reversal × 1~2 bar timeout` 这种单一、短时、可程序化的 reversal alpha；
3. 这些对象都比原 `Rank 11` 的宽泛 `causal extrema pattern gate` 更具体，也更像完整策略骨架；
...
2. 最近仍然最强的旁证是在把主题推向新的 **event-driven / reversal raw-alpha family**，而不是支持原 `causal extrema gate` 的 residual pocket；
...
- short note: `hard park；可救信号只存在于更窄的 event-driven / reversal raw-alpha family，而不属于旧 Rank 11 causal-extrema pattern gate 本体，因此当前不诚实 draft Rank 11b`
