#!/usr/bin/env python3
"""
Phase 2c Carry Harvest Backtest (Fixed Entry Timing)
=====================================================
修复开仓时间问题的回测脚本。

关键修复：
1. stall_t2: T+2之后开仓（hours_from_event >= 48）
2. stall_t3: T+3之后开仓（hours_from_event >= 72）
3. immediate_reversal: T+1之后开仓（hours_from_event >= 24）
4. continuation: T+1之后开仓（hours_from_event >= 24）

避免前视偏差：确保开仓时间在结构确认之后
"""
import pandas as pd
import numpy as np
import os, time, json
...
# ── Structure-specific entry timing ────────────────────────────────────────
# 根据v1.5的结构分类逻辑，确定每种结构的最早开仓时间
STRUCTURE_ENTRY_HOURS = {
    'immediate_reversal': 24,  # T+1之后开仓
    'stall_t2': 48,            # T+2之后开仓
    'stall_t3': 72,            # T+3之后开仓
    'continuation': 24,        # T+1之后开仓
    'melt_up': 24,             # T+1之后开仓
}

# ── Load data ──────────────────────────────────────────────────────────────
print("=" * 70)
print("PHASE 2C CARRY HARVEST BACKTEST (FIXED ENTRY TIMING)")
print("=" * 70)
...
print("\n[2/3] Preparing data with correct entry timing...")
# 计算每种结构的最早开仓时间
panel['min_entry_hour'] = panel['ev_structure'].map(STRUCTURE_ENTRY_HOURS)
panel['min_entry_hour'] = panel['min_entry_hour'].fillna(24)  # 默认T+1

# 筛选：只保留开仓时间之后的数据
panel = panel[panel['hours_from_event'] >= panel['min_entry_hour']]
print(f"  Rows after entry timing filter: {len(panel):,}")
...
def scan_funding_structure(funding_pctl_thresh, structure_filter=None, hold_hours=8):
    ...
    for (sym, ed), group in filtered.groupby(['symbol', 'event_date']):
        ...
        # IMPORTANT: Only sum funding at settlement times, not every hour
        # funding is only settled at specific intervals (mostly every 8h)
        hold_data = group.loc[entry_idx:exit_idx]
        unique_settlements = hold_data.drop_duplicates(subset='funding_settlement_ts')
        funding_sum = unique_settlements['funding_rate'].sum()
    ...
    entry_hour_stats = {}
    for eh in sorted(td['entry_hour'].unique()):
        eh_mask = td['entry_hour'] == eh
        eh_net = net[eh_mask]
        if len(eh_net) >= 10:
            entry_hour_stats[int(eh)] = {
                'n': int(eh_mask.sum()),
                'mean': float(eh_net.mean()),
                'win_rate': float((eh_net > 0).mean()),
            }

        # Calculate trades per year for proper annualization
        years_span = (td['year'].max() - td['year'].min() + 1)
        trades_per_year = len(net) / years_span if years_span > 0 else len(net)
        ...
        return {
            ...
            'sharpe': float(net.mean() / net.std() * np.sqrt(trades_per_year)) if net.std() > 0 else 0,
        'funding_mean': float(td['funding_at_entry'].mean()),
        'entry_hour_mean': float(td['entry_hour'].mean()),
        'year_stats': year_stats,
        'struct_stats': struct_stats,
        'bucket_stats': bucket_stats,
        'entry_hour_stats': entry_hour_stats,
    }
...
results_path = OUT_DIR / "param_scan_results_fixed.json"
...
for r in all_results:
    summary_rows.append({
        ...
        'entry_hour_mean': r['entry_hour_mean'],
    })

summary_df = pd.DataFrame(summary_rows)
summary_path = OUT_DIR / "param_scan_summary_fixed.csv"
summary_df.to_csv(summary_path, index=False)
print(f"  Saved: {summary_path}")

# Find top variants
print("\n" + "=" * 70)
print("TOP VARIANTS BY NET MEAN (FIXED ENTRY TIMING)")
print("=" * 70)
...
for i, row in top_variants.iterrows():
    ...
    print(f"  Entry hour mean: {row['entry_hour_mean']:.1f}")
    ...
    if best_result:
        ...
        if 'entry_hour_stats' in best_result and best_result['entry_hour_stats']:
            ...
            for eh, stats in sorted(best_result['entry_hour_stats'].items()):
                print(f"  h={eh}: {stats['n']} trades, {stats['mean']*100:.2f}% net, {stats['win_rate']*100:.1f}% win")

print(f"\n{'=' * 70}")
print("COMPARISON WITH PREVIOUS RESULTS")
print(f"{'=' * 70}")
print("\nPrevious results (with look-ahead bias):")
print("  Best: funding_pctl < 0.10 + stall_t3 + 24h = +5.40% net, 65.5% win")
print("\nCurrent results (fixed entry timing):")
if len(top_variants) > 0:
    best = top_variants.iloc[0]
    print(f"  Best: funding_pctl < {best['funding_pctl_thresh']:.2f} + {best['structure_filter']} + {best['hold_hours']}h = {best['net_mean']*100:.2f}% net, {best['win_rate']*100:.1f}% win")
else:
...
print("NEXT STEPS")
print(f"{'=' * 70}")
print("1. Run build_phase2c_report_fixed.py to generate HTML report")
print("2. Compare with previous results to assess impact of look-ahead bias")
print("3. Review entry hour distribution for each structure type")
print("4. Consider paper lane promotion if results are still positive")
