...
SITE_PATH = ROOT / "reports" / "site" / "paper" / "rank213_largecap_xs_jump_veto_monthly_volume_percentile_gate_review.html"
...
SUMMARY_PATH = ART_DIR / "rank213_monthly_volume_percentile_gate_review_summary.json"
GRID_PATH = ART_DIR / "rank213_monthly_volume_percentile_gate_review_grid.csv"
BUCKET_PATH = ART_DIR / "rank213_monthly_volume_percentile_gate_review_strength_buckets.csv"
FEATURE_BUCKET_PATH = ART_DIR / "rank213_monthly_volume_percentile_gate_review_feature_buckets.csv"
DETAIL_OUT_PATH = ART_DIR / "rank213_monthly_volume_percentile_gate_review_detail.csv"
SUBPERIOD_GRID_PATH = ART_DIR / "rank213_monthly_volume_percentile_gate_review_subperiod_grid.csv"
DECISION_PATH = ART_DIR / "rank213_monthly_volume_percentile_gate_review_frozen_decision.json"
...
def build_review() -> dict:
    ...
    summary = {
        "scope": "monthly volume rebuild percentile gate review using causal monthly-rebuild feature history only",
        "methodology": {
            "universe_definition": "For each historical rebalance t, first use the month containing t to define the as-of monthly volume-proxy universe; then use that row's own monthly-rebuild features.",
        ...
        "anchors": {
            ...
            "monthly_raw_gate_on_rate_pct": float(monthly_summary.get("metrics", {}).get("monthly_volume_rebuild", {}).get("baseline_plus_veto_plus_gate", {}).get("gate_on_rate_pct", np.nan)),
            "monthly_raw_gate_net_cum_pct": float(monthly_summary.get("metrics", {}).get("monthly_volume_rebuild", {}).get("baseline_plus_veto_plus_gate", {}).get("net_cum_pct", np.nan)),
        },
...
"""
    ...
    method_list = f"""
    <ul>
      <li><b>Universe 口径固定：</b>对每个历史时点 <code>t</code>，先用 <code>t</code> 所在月份的 monthly volume-proxy rebuild 定义当时 universe，再读取该行对应的 monthly-rebuild feature；不是拿“当前这组币”回头跑历史 percentile。</li>
...
<head>
  ...
  <title>Rank213 monthly volume percentile gate review</title>
  <style>
...
<body><div class='wrap'>
  <div class='card'>
    <h1>Rank213：monthly volume rebuild × percentile gate review</h1>
    <p class='muted'>这页回答的问题不是“当前 live/paper runner 在跑什么”，而是：<b>如果把 monthly rebuild 这条证据面的 gate 改成 percentile / adaptive 强度口径，它在 0~100 强度全范围下到底长什么样，以及阈值放哪更合适。</b></p>
    <p><a href='/momentum/paper/rank213_largecap_xs_jump_veto.html'>返回主页面</a> · <a href='/momentum/paper/rank213_largecap_xs_jump_veto_monthly_volume_universe_rebuild.html'>monthly volume rebuild 原始页面</a> · <a href='/momentum/paper/rank213_largecap_xs_jump_veto_formal_strategy_review.html'>formal strategy review</a></p>
  </div>
