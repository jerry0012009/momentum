...
ART_DIR = ROOT / "reports" / "artifacts" / "scout_vol_regime_midband_15m"
SITE_DIR = ROOT / "reports" / "site" / "factors" / "scout_vol_regime_midband_15m"
...
PRIMARY_LABEL = "rv_midband_q20_80"
VARIANT_LABELS = [BASELINE_LABEL, "no_high_vol_extreme", PRIMARY_LABEL, "rv_midband_q30_70"]
PARAM_GRID = [
    {"label": "q10_90", "q_low": 0.10, "q_high": 0.90, "vol_window_1h": 12, "quantile_window_1h": 72},
    {"label": "q20_80", "q_low": 0.20, "q_high": 0.80, "vol_window_1h": 12, "quantile_window_1h": 72},
    {"label": "q30_70", "q_low": 0.30, "q_high": 0.70, "vol_window_1h": 12, "quantile_window_1h": 72},
    {"label": "q20_80_fastq", "q_low": 0.20, "q_high": 0.80, "vol_window_1h": 12, "quantile_window_1h": 48},
    {"label": "q20_80_slowrv", "q_low": 0.20, "q_high": 0.80, "vol_window_1h": 18, "quantile_window_1h": 72},
]
...
def build_vol_gate_signals(
    bars: pd.DataFrame,
    *,
    q_low: float,
    q_high: float,
    vol_window_1h: int = 12,
    quantile_window_1h: int = 72,
) -> pd.DataFrame:
    base = build_baseline_signals(bars)
    market = bars[["timestamp", "close"]].copy().set_index("timestamp")
    market = market.resample("1h").last().dropna().reset_index()
    market["ret_1h"] = market["close"].pct_change()
    market["rv_1h"] = market["ret_1h"].rolling(vol_window_1h, min_periods=vol_window_1h).std(ddof=0)
    min_q = max(vol_window_1h, quantile_window_1h // 3)
    market["rv_q_low"] = market["rv_1h"].rolling(quantile_window_1h, min_periods=min_q).quantile(float(q_low))
    market["rv_q_high"] = market["rv_1h"].rolling(quantile_window_1h, min_periods=min_q).quantile(float(q_high))

    out = pd.merge_asof(
        base.sort_values("timestamp"),
        market[["timestamp", "rv_1h", "rv_q_low", "rv_q_high"]].sort_values("timestamp"),
        on="timestamp",
        direction="backward",
    )
    out[["rv_1h", "rv_q_low", "rv_q_high"]] = out[["rv_1h", "rv_q_low", "rv_q_high"]].astype(float)
    out["vol_known"] = (out["rv_1h"].notna() & out["rv_q_high"].notna()).astype(int)
    out["vol_not_extreme"] = ((out["rv_1h"] <= out["rv_q_high"]) | out["rv_q_high"].isna()).astype(int)
    out["vol_midband"] = (
        ((out["rv_1h"] >= out["rv_q_low"]) & (out["rv_1h"] <= out["rv_q_high"]))
        | out["rv_q_high"].isna()
    ).astype(int)
    return out
...
def variant_from_vol_base(vol_sig: pd.DataFrame, variant: str) -> tuple[pd.DataFrame, float]:
    sig = vol_sig.copy()
    if variant == "no_high_vol_extreme":
        gate = sig["vol_not_extreme"] == 1
    elif variant in {"rv_midband_q20_80", "rv_midband_q30_70"}:
        gate = sig["vol_midband"] == 1
    else:
        raise ValueError(f"unsupported variant: {variant}")
    sig["long_signal"] = ((sig["long_signal"] == 1) & gate).astype(int)
    sig["short_signal"] = ((sig["short_signal"] == 1) & gate).astype(int)
    sig["long_filtered_out"] = ((sig["long_signal"] == 0) & (sig["base_long_signal"] == 1)).astype(int)
    sig["short_filtered_out"] = ((sig["short_signal"] == 0) & (sig["base_short_signal"] == 1)).astype(int)
    gate_pass_ratio = float(gate.mean()) if len(gate) else np.nan
    return sig, gate_pass_ratio



def evaluate_one(bars: pd.DataFrame, *, variant: str, cost_bps: float, vol_params: dict | None = None) -> tuple[dict, pd.DataFrame]:
    if variant == BASELINE_LABEL:
        ...
        if vol_params is None:
            if variant == "rv_midband_q30_70":
                vol_params = {"q_low": 0.30, "q_high": 0.70, "vol_window_1h": 12, "quantile_window_1h": 72}
            else:
                vol_params = {"q_low": 0.20, "q_high": 0.80, "vol_window_1h": 12, "quantile_window_1h": 72}
        vol_sig = build_vol_gate_signals(bars, **vol_params)
        vol_sig["base_long_signal"] = vol_sig["long_signal"].astype(int)
        vol_sig["base_short_signal"] = vol_sig["short_signal"].astype(int)
        sig, gate_pass_ratio = variant_from_vol_base(vol_sig, variant)
        ...
        [
            {
                ...
                "value": "EMA 当前 waiting_not_due；Rank 17 与 Rank 2 都是 P3 且当前没有真实 append/review need，因此当前边际价值最高的动作是把 Rank 23 从 source intake 直接推进到 clean replication + Light Stability Pack verdict。",
                "why_it_matters": "当前 desk 需要更快回答 vol/cost survival gate 到底是值得升格还是应直接 park。",
                "operator_rule": "本轮只跑 Rank 23 一条线，不并行打开其他 fresh intake。",
            },
            {
                "section": "candidate",
                "item": "candidate_id",
                "value": "rank23_vol_regime_midband_gate",
            ...
            },
            {
                ...
                "value": "Svogun & Bazán-Palomino (2022) + 本地 `svogun2022_cost_regime_experiment`，把『高摩擦极端 vol pocket 应避免、mid-band 可能留下 survival』压成 baseline multi-tf momentum 的环境 gate。",
                "why_it_matters": "它不是新 alpha 主体，而是检验波动 regime 过滤有没有净增量。",
                "operator_rule": "trade on = baseline multi-tf momentum 同向且 realized-vol gate 放行；trade off = baseline 消失或 vol gate 不放行。",
            },
            {
                "section": "scope",
                "item": "market_timeframe",
                "value": "BTC-USD / ETH-USD / SOL-USD | Binance 120d cache | 15m execution + 1h realized-vol gate",
            ...
            },
            {
                ...
                "value": "baseline_mtf / no_high_vol_extreme / rv_midband_q20_80 / rv_midband_q30_70",
                "why_it_matters": "直接回答『只去掉高 vol 极端』与『明确只留中间 vol 带』是否比 baseline 更诚实。",
            ...
            },
            {
                ...
                "value": "1h realized-vol 只使用历史已完成 bar 的 rolling quantile；默认下一根 15m bar 执行；没有 lookahead / repaint / data leakage。",
                "why_it_matters": "先过 causality，再谈 replication 结果。",
                "operator_rule": "禁止用未来 regime 标签或后验 bubble 分类直接入场。",
            },
        ]
    )
...
<head>
  ...
  <title>Scout Seat · vol regime mid-band gate · 15m crypto</title>
  <style>
...
<body>
  ...
  <h1>Scout Seat · vol regime mid-band gate · 15m crypto</h1>
  <p class="muted">生成时间：{escape(str(meta['generated_at_utc']))} ｜ Rank 23 clean replication + Light Stability Pack。</p>
  ...
  <div class="card">
    <h2>cross-asset stability（主变体 q20_80）</h2>
  ...
  <div class="card">
    <h2>time stability（主变体 q20_80）</h2>
  ...
  <div class="card">
    <h2>parameter stability（vol-band 邻域）</h2>
  ...
  <div class="card">
    <h2>cost / trade-count stability（主变体 q20_80）</h2>
    ...
    <p class="muted">artifact：<code>reports/artifacts/scout_vol_regime_midband_15m/</code></p>
  </div>
</body>
</html>
'''
...
def main() -> None:
    ...
    for cfg in PARAM_GRID:
        cfg_rows = []
        label = f"rv_midband_{cfg['label']}"
        eval_params = {k: v for k, v in cfg.items() if k != "label"}
        for asset, symbol in ASSETS.items():
            bars = load_cached_bars(symbol, asset)
            s, _ = evaluate_one(bars, variant=PRIMARY_LABEL, cost_bps=PRIMARY_COST, vol_params=eval_params)
            cfg_rows.append(s)
        agg = aggregate(pd.DataFrame(cfg_rows), "variant")
        row = agg.iloc[0].to_dict()
        row["param_label"] = label
    ...
    primary6 = summary_df.loc[summary_df["variant"] == PRIMARY_LABEL].iloc[0]
    noextreme6 = summary_df.loc[summary_df["variant"] == "no_high_vol_extreme"].iloc[0]
    ...
    verdict_reasons = []
    if primary6["mean_total_return"] <= 0:
        verdict = "park / evidence pool"
        verdict_reasons.append("主变体 6bps aggregate 仍为负")
    if primary6["positive_asset_ratio"] < (2.0 / 3.0):
        verdict = "park / evidence pool"
        verdict_reasons.append("跨标的 positive_asset_ratio < 2/3")
    if p10["mean_total_return"] <= 0:
        verdict = "park / evidence pool"
        verdict_reasons.append("10bps cost survival 未过")
    if primary6["mean_no_trade_ratio"] > 0.70:
        verdict = "park / evidence pool"
        verdict_reasons.append("no_trade_ratio 过高")
    if best_param["mean_total_return"] <= 0:
        verdict = "park / evidence pool"
        verdict_reasons.append("参数邻域没有正 pocket")

    verdict_text = (
        "当前 Rank 23 vol regime mid-band gate 的更诚实读法仍是 park / evidence pool，不进入 paper candidate pool。"
        if verdict != "paper candidate"
        else "当前 Rank 23 vol regime mid-band gate 已满足最小 paper candidate 条件，可进入 paper candidate pool。"
    )
    evidence1 = (
        f"主变体 {PRIMARY_LABEL} 在 6bps/side 下跨资产 mean_total_return={pct(primary6['mean_total_return'])}，"
        f"positive_asset_ratio={pct(primary6['positive_asset_ratio'])}，mean_trades={num(primary6['mean_trades'],1)}，mean_no_trade_ratio={pct(primary6['mean_no_trade_ratio'])}。"
    )
    evidence2 = (
        f"baseline_mtf 6bps={pct(baseline6['mean_total_return'])}；no_high_vol_extreme 6bps={pct(noextreme6['mean_total_return'])}；"
        f"10bps={pct(p10['mean_total_return'])}，15bps={pct(p15['mean_total_return'])}；time 正收益 bucket={positive_buckets}/{len(time_df)}；最佳邻域={best_param['param_label']}({pct(best_param['mean_total_return'])})。"
    )

    admission_df = pd.DataFrame(
        [
            {
                "candidate_id": "rank23_vol_regime_midband_gate",
                "verdict": verdict,
                "primary_variant": PRIMARY_LABEL,
                "primary_cost_bps_per_side": PRIMARY_COST,
                "mean_total_return": primary6["mean_total_return"],
                "positive_asset_ratio": primary6["positive_asset_ratio"],
                "mean_trades": primary6["mean_trades"],
                "mean_no_trade_ratio": primary6["mean_no_trade_ratio"],
                "baseline_6bps_mean_total_return": baseline6["mean_total_return"],
                "no_high_vol_extreme_6bps_mean_total_return": noextreme6["mean_total_return"],
                "cost_10_mean_total_return": p10["mean_total_return"],
                "best_neighbor": best_param["param_label"],
                "best_neighbor_mean_total_return": best_param["mean_total_return"],
                "decision_note": "；".join(verdict_reasons) if verdict_reasons else "通过最小门槛",
            }
        ]
    )
        ...
        [
            {
                ...
                "candidate_id": "rank23_vol_regime_midband_gate",
                "source_anchor": "Svogun & Bazán-Palomino (2022) + local cost regime experiment",
    ...
    print(f"Wrote scout vol regime artifacts to {ART_DIR}")
