...
THRESHOLDS_PATH = ROOT / 'reports' / 'artifacts' / 'quant_digests' / 'ewmac_breakout_alignment_20260323' / 'thresholds.json'
FEATURE_DIR = ROOT / 'reports' / 'artifacts' / 'scout_rank74_adx_er_trend_readiness_15m'
ART_DIR = ROOT / 'reports' / 'artifacts' / 'scout_rank151_fib_retest_family_honest_gate_15m'
SITE_DIR = ROOT / 'reports' / 'site' / 'factors' / 'scout_rank151_fib_retest_family_honest_gate_15m'
READING_PATH = ROOT / 'reports' / 'site' / 'reading' / 'repo_scout' / 'rank151_fib_retest_family_honest_gate.html'
...
def build_events() -> pd.DataFrame:
    rows: list[dict] = []
    for asset in ASSETS:
        df = pd.read_csv(FEATURE_DIR / f'{asset}_feature_frame.csv')
        ...
        ema32 = close.ewm(span=32, adjust=False).mean()
        ema96 = close.ewm(span=96, adjust=False).mean()
        signal_idx = df.index[df['fib_retest_long_signal'].fillna(False).astype(bool)]
        for idx in signal_idx:
            exit_idx = idx + HOLD_BARS
            if exit_idx >= len(df):
                continue
            if pd.isna(atr14.iloc[idx]) or float(atr14.iloc[idx]) == 0.0:
                continue
            entry = float(close.iloc[idx])
            exit_px = float(close.iloc[exit_idx])
            signed_bps = (exit_px - entry) / entry * 10000.0
            align_score = float((ema32.iloc[idx] - ema96.iloc[idx]) / atr14.iloc[idx])
            rows.append({
    ...
    events = pd.DataFrame(rows).sort_values(['ts', 'asset']).reset_index(drop=True)
    return events
...
def main() -> None:
    ...
    for cost in COSTS:
        for variant in VARIANTS:
            ...
            for asset, asset_df in events.groupby('asset'):
                signal_count = len(asset_df)
                picked = subset[subset['asset'] == asset]
                asset_rows.append(summarize(picked, signal_count, cost, variant, asset))
    ...
    verdict = (
        'second family passed：fib retest 上 band-pass 明显优于 baseline / hard-positive；'
        'Rank 151 不再只是 breakout-short 特例，已拿到跨第二条 desk family 的 shared-gate 证据。'
        if band['mean_net_bps'] > base['mean_net_bps'] and band['positive_asset_ratio'] >= base['positive_asset_ratio']
        else 'second family failed：当前还看不出能跨第二条 desk family 复现。'
    )
    ...
    meta = {
        ...
        'source': 'scout_rank74_adx_er_trend_readiness_15m/*_feature_frame.csv',
        'hold_bars': HOLD_BARS,
    ...
    title = 'Rank 151 · fib retest second-family honest gate'
    ...
    body = f"""
<h1>{escape(title)}</h1>
<p class='muted'>生成时间：{escape(meta['generated_at_utc'])} ｜ family：fib retest ｜ 来源：rank74 feature frame（BTC/ETH/SOL 15m）｜ hold={HOLD_BARS} bars ｜ frozen 阈值：q20={q20:.4f}, q80={q80:.4f} ｜ 成本层：6/10/15 bps per side</p>
...
<li>这是第二条 family 复核，不是重新优化阈值；<code>q20/q80</code> 直接复用 breakout digest 冻结值。</li>
<li>样本较小（fib retest 事件本来就少），因此它更像 replication evidence，而不是最终 deploy 结论。</li>
<li>如果第二 family 也过，下一轮最值钱动作不再是补第三条 family，而是上更正式的 rolling / split 稳定性。</li>
</ul></div>
"""
