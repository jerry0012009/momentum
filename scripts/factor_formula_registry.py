...
REGISTRY: list[FactorSpec] = [
    ...
    # Public Alpha101 early OHLCV/VWAP panel batch 08
    FactorSpec(
        factor_id="wq101_alpha1", family="wq101",
        required_columns=["close"], lookback_window=25,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#1: rank(ts_argmax(signedpower(returns<0 ? stddev(returns,20) : close,2),5))-0.5; panel rank semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha1(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha2", family="wq101",
        required_columns=["open", "close", "volume"], lookback_window=8,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#2: -correlation(rank(delta(log(volume),2)), rank((close-open)/open), 6); panel rank semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha2(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha3", family="wq101",
        required_columns=["open", "volume"], lookback_window=10,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#3: -correlation(rank(open), rank(volume), 10); panel rank semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha3(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha4", family="wq101",
        required_columns=["low"], lookback_window=9,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#4: -ts_rank(rank(low), 9); rank is cross-sectional, ts_rank is per symbol",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha4(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha5", family="wq101",
        required_columns=["open", "close", "volume", "quote_volume"], lookback_window=10,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#5: rank(open-mean(vwap,10)) * -abs(rank(close-vwap)); VWAP derived from quote_volume/volume",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha5(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha7", family="wq101",
        required_columns=["close", "volume"], lookback_window=67,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#7: if adv20 < volume then -ts_rank(abs(delta(close,7)),60)*sign(delta(close,7)) else -1",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha7(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha8", family="wq101",
        required_columns=["open", "close"], lookback_window=15,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#8: -rank(sum(open,5)*sum(returns,5)-delay(sum(open,5)*sum(returns,5),10)); panel rank semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha8(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha10", family="wq101",
        required_columns=["close"], lookback_window=5,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#10: rank(conditional signed close delta over rolling four-bar delta extrema)",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha10(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha11", family="wq101",
        required_columns=["close", "volume", "quote_volume"], lookback_window=4,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#11: (rank(ts_max(vwap-close,3))+rank(ts_min(vwap-close,3)))*rank(delta(volume,3))",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha11(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha13", family="wq101",
        required_columns=["close", "volume"], lookback_window=5,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#13: -rank(covariance(rank(close), rank(volume), 5)); panel rank semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha13(bars),
    ),
    # Public Alpha101 early OHLCV/VWAP panel batch 09
    FactorSpec(
        factor_id="wq101_alpha14", family="wq101",
        required_columns=["open", "close", "volume"], lookback_window=10,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#14: -rank(delta(returns,3))*correlation(open,volume,10); panel rank semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha14(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha15", family="wq101",
        required_columns=["high", "volume"], lookback_window=5,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#15: -sum(rank(correlation(rank(high),rank(volume),3)),3); panel rank semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha15(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha16", family="wq101",
        required_columns=["high", "volume"], lookback_window=5,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#16: -rank(covariance(rank(high),rank(volume),5)); panel rank semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha16(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha17", family="wq101",
        required_columns=["close", "volume"], lookback_window=24,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#17: -rank(ts_rank(close,10))*rank(delta(delta(close,1),1))*rank(ts_rank(volume/adv20,5))",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha17(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha18", family="wq101",
        required_columns=["open", "close"], lookback_window=10,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#18: -rank(stddev(abs(close-open),5)+(close-open)+correlation(close,open,10))",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha18(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha19", family="wq101",
        required_columns=["close"], lookback_window=251,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#19: -sign((close-delay(close,7))+delta(close,7))*(1+rank(1+sum(returns,250)))",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha19(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha20", family="wq101",
        required_columns=["open", "high", "low", "close"], lookback_window=2,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#20: -rank(open-delay(high,1))*rank(open-delay(close,1))*rank(open-delay(low,1))",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha20(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha22", family="wq101",
        required_columns=["high", "close", "volume"], lookback_window=20,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#22: -delta(correlation(high,volume,5),5)*rank(stddev(close,20))",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha22(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha26", family="wq101",
        required_columns=["high", "volume"], lookback_window=11,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#26: -ts_max(correlation(ts_rank(volume,5),ts_rank(high,5),5),3)",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha26(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha27", family="wq101",
        required_columns=["volume", "quote_volume"], lookback_window=7,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#27: rank(mean(correlation(rank(volume),rank(vwap),6),2)) > 0.5 ? -1 : 1",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha27(bars),
    ),
    # Public Alpha101 OHLCV/VWAP/ADV panel batch 10
    FactorSpec(
        factor_id="wq101_alpha29", family="wq101",
        required_columns=["close"], lookback_window=11,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#29: nested close-delta rank scale plus ts_rank(delay(-returns,6),5); panel rank semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha29(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha31", family="wq101",
        required_columns=["close", "low", "volume"], lookback_window=31,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#31: triple-ranked decayed close-delta branch plus short delta rank and sign(scale(correlation(adv20,low,12)))",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha31(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha36", family="wq101",
        required_columns=["open", "close", "volume", "quote_volume"], lookback_window=200,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#36: weighted blend of candle/volume correlation, return rank, VWAP/ADV correlation, and close mean state",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha36(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha39", family="wq101",
        required_columns=["close", "volume"], lookback_window=251,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#39: -rank(delta(close,7)*(1-rank(decay_linear(volume/adv20,9))))*(1+rank(sum(returns,250)))",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha39(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha57", family="wq101",
        required_columns=["close", "volume", "quote_volume"], lookback_window=31,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#57: -(close-vwap)/decay_linear(rank(ts_argmax(close,30)),2); vwap derived from quote_volume/volume",
        compute_scope="panel",
    ...
    FactorSpec(
        ...
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#62: -1 boolean gate comparing VWAP/sum(adv20,22) correlation rank with OHLC rank condition",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha62(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha64", family="wq101",
        required_columns=["open", "high", "low", "volume", "quote_volume"], lookback_window=149,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#64: -1 boolean gate comparing open-low/adv120 correlation rank with blended mid/VWAP delta rank",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha64(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha66", family="wq101",
        required_columns=["open", "high", "low", "volume", "quote_volume"], lookback_window=17,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#66: negative sum of decayed VWAP-delta rank and decayed intrabar-location ts-rank",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha66(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha71", family="wq101",
        required_columns=["open", "low", "close", "volume", "quote_volume"], lookback_window=229,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#71: max of decayed close/adv180 correlation ts-rank and decayed low-open/VWAP rank branch",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha71(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha72", family="wq101",
        required_columns=["high", "low", "volume", "quote_volume"], lookback_window=49,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#72: ratio of decayed mid/adv40 correlation rank to decayed VWAP/volume ts-rank correlation rank",
        compute_scope="panel",
    ...
    # Public Alpha101 panel OHLCV/VWAP batch 03
    FactorSpec(
        factor_id="wq101_alpha32", family="wq101",
        required_columns=["close", "volume", "quote_volume"], lookback_window=235,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#32: scale(mean(close,7)-close)+20*scale(correlation(vwap,delay(close,5),230)); panel rank/scale semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha32(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha33", family="wq101",
        required_columns=["open", "close"], lookback_window=1,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#33: rank(-1*(1-open/close)); panel rank semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha33(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha37", family="wq101",
        required_columns=["open", "close"], lookback_window=201,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#37: rank(correlation(delay(open-close,1),close,200))+rank(open-close); panel rank semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha37(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha38", family="wq101",
        required_columns=["open", "close"], lookback_window=10,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#38: -rank(ts_rank(close,10))*rank(close/open); panel rank semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha38(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha44", family="wq101",
        required_columns=["high", "volume"], lookback_window=5,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#44: -correlation(high,rank(volume),5); rank is cross-sectional per timestamp",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha44(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha45", family="wq101",
        required_columns=["close", "volume"], lookback_window=25,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#45: -rank(mean(delay(close,5),20))*correlation(close,volume,2)*rank(correlation(sum(close,5),sum(close,20),2)); panel rank semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha45(bars),
    ),
    # Public Alpha101 panel OHLCV/VWAP batch 04
    FactorSpec(
        factor_id="wq101_alpha34", family="wq101",
        required_columns=["close"], lookback_window=6,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#34: rank((1-rank(std(returns,2)/std(returns,5)))+(1-rank(delta(close,1)))); panel rank semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha34(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha40", family="wq101",
        required_columns=["high", "volume"], lookback_window=10,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#40: -rank(stddev(high,10))*correlation(high,volume,10); rank is cross-sectional per timestamp",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha40(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha42", family="wq101",
        required_columns=["close", "volume", "quote_volume"], lookback_window=1,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#42: rank(vwap-close)/rank(vwap+close); vwap derived as quote_volume/volume; panel rank semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha42(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha50", family="wq101",
        required_columns=["volume", "quote_volume"], lookback_window=9,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#50: -ts_max(rank(correlation(rank(volume),rank(vwap),5)),5); panel rank semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha50(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha55", family="wq101",
        required_columns=["high", "low", "close", "volume"], lookback_window=17,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#55: -correlation(rank((close-ts_min(low,12))/(ts_max(high,12)-ts_min(low,12))),rank(volume),6); panel rank semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha55(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha60", family="wq101",
        required_columns=["high", "low", "close", "volume"], lookback_window=10,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#60: -((2*scale(rank(close-location-volume)))-scale(rank(ts_argmax(close,10)))); panel rank/scale semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha60(bars),
    ),
    # Public Alpha101 panel OHLCV/VWAP batch 05
    FactorSpec(
        factor_id="wq101_alpha25", family="wq101",
        required_columns=["high", "close", "volume", "quote_volume"], lookback_window=20,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#25: rank((-returns*adv20*vwap)*(high-close)); vwap derived as quote_volume/volume; rank is cross-sectional per timestamp",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha25(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha28", family="wq101",
        required_columns=["high", "low", "close", "volume"], lookback_window=24,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#28: scale(correlation(adv20,low,5)+((high+low)/2)-close); panel scale semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha28(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha30", family="wq101",
        required_columns=["close", "volume"], lookback_window=20,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#30: (1-rank(sum of last three close-delta signs))*sum(volume,5)/sum(volume,20); rank is cross-sectional per timestamp",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha30(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha35", family="wq101",
        required_columns=["high", "low", "close", "volume"], lookback_window=33,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#35: ts_rank(volume,32)*(1-ts_rank(close+high-low,16))*(1-ts_rank(returns,32)); adapted to 1h panel",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha35(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha43", family="wq101",
        required_columns=["close", "volume"], lookback_window=39,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#43: ts_rank(volume/adv20,20)*ts_rank(-delta(close,7),8); adapted to 1h panel",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha43(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha52", family="wq101",
        required_columns=["low", "close", "volume"], lookback_window=241,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#52: -delta(ts_min(low,5),5)*rank((sum(returns,240)-sum(returns,20))/220)*ts_rank(volume,5); rank is cross-sectional per timestamp",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha52(bars),
    ),
    # Public Alpha101 panel OHLCV/VWAP batch 06
    FactorSpec(
        factor_id="wq101_alpha47", family="wq101",
        required_columns=["high", "close", "volume", "quote_volume"], lookback_window=20,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#47: rank(1/close)*volume/adv20*high*rank(high-close)/(sum(high,5)/5)-rank(vwap-delay(vwap,5)); panel rank semantics preserved",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha47(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha61", family="wq101",
        required_columns=["volume", "quote_volume"], lookback_window=197,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#61: rank(vwap-ts_min(vwap,16)) < rank(correlation(vwap,adv180,18)); boolean panel formula",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha61(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha65", family="wq101",
        required_columns=["open", "volume", "quote_volume"], lookback_window=73,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#65: -1*(rank(correlation(open/vwap blend,sum(adv60,9),6)) < rank(open-ts_min(open,14))); boolean panel formula",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha65(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha68", family="wq101",
        required_columns=["high", "low", "close", "volume"], lookback_window=37,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#68: -1*(ts_rank(correlation(rank(high),rank(adv15),9),14) < rank(delta(close/low blend,1))); boolean panel formula",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha68(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha74", family="wq101",
        required_columns=["high", "close", "volume", "quote_volume"], lookback_window=80,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#74: -1*(rank(correlation(close,sum(adv30,37),15)) < rank(correlation(rank(high/vwap blend),rank(volume),11))); boolean panel formula",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha74(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha75", family="wq101",
        required_columns=["low", "volume", "quote_volume"], lookback_window=61,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#75: rank(correlation(vwap,volume,4)) < rank(correlation(rank(low),rank(adv50),12)); boolean panel formula",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha75(bars),
    ),
    # Public Alpha101 panel OHLCV/VWAP/ADV decay batch 07
    FactorSpec(
        factor_id="wq101_alpha77", family="wq101",
        required_columns=["high", "low", "volume", "quote_volume"], lookback_window=47,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#77: min(rank(decay_linear(mid-vwap,20)), rank(decay_linear(correlation(mid,adv40,3),6))); decimal windows mapped to nearest integer",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha77(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha78", family="wq101",
        required_columns=["low", "volume", "quote_volume"], lookback_window=65,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#78: rank(correlation(sum(low/vwap blend,20),sum(adv40,20),7)) ^ rank(correlation(rank(vwap),rank(volume),6))",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha78(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha83", family="wq101",
        required_columns=["high", "low", "close", "volume", "quote_volume"], lookback_window=7,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#83: rank(delay((high-low)/(sum(close,5)/5),2))*rank(rank(volume)) / (((high-low)/(sum(close,5)/5))/(vwap-close))",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha83(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha85", family="wq101",
        required_columns=["high", "low", "close", "volume"], lookback_window=39,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#85: rank(correlation(high/close blend,adv30,10)) ^ rank(correlation(ts_rank(mid,4),ts_rank(volume,10),7))",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha85(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha86", family="wq101",
        required_columns=["open", "close", "volume", "quote_volume"], lookback_window=58,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#86: -1*(ts_rank(correlation(close,sum(adv20,15),6),20) < rank(close-vwap)); boolean panel formula",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha86(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha88", family="wq101",
        required_columns=["open", "high", "low", "close", "volume"], lookback_window=95,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#88: min(rank(decay_linear(rank(open)+rank(low)-rank(high)-rank(close),8)), ts_rank(decay_linear(correlation(ts_rank(close,8),ts_rank(adv60,21),8),7),3))",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha88(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha92", family="wq101",
        required_columns=["open", "high", "low", "close", "volume"], lookback_window=49,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#92: min(ts_rank(decay_linear(mid+close < low+open,15),19), ts_rank(decay_linear(correlation(rank(low),rank(adv30),8),7),7))",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha92(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha94", family="wq101",
        required_columns=["volume", "quote_volume"], lookback_window=82,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#94: -rank(vwap-ts_min(vwap,12)) ^ ts_rank(correlation(ts_rank(vwap,20),ts_rank(adv60,4),18),3)",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha94(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha95", family="wq101",
        required_columns=["open", "high", "low", "volume"], lookback_window=81,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#95: rank(open-ts_min(open,12)) < ts_rank(rank(correlation(sum(mid,19),sum(adv40,19),13))^5,12); boolean panel formula",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha95(bars),
    ),
    FactorSpec(
        factor_id="wq101_alpha99", family="wq101",
        required_columns=["high", "low", "volume"], lookback_window=87,
        expected_direction="conditional",
        notes="WorldQuant 101 Alpha#99: -1*(rank(correlation(sum(mid,20),sum(adv60,20),9)) < rank(correlation(low,volume,6))); boolean panel formula",
        compute_scope="panel",
        panel_compute_fn=lambda bars: __import__('alpha101_panel_ops').compute_wq101_alpha99(bars),
    ),
    # Batch 1: Alpha158
    FactorSpec(
