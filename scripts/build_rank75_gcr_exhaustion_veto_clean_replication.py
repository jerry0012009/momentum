...
ART_DIR = ROOT / "reports" / "artifacts" / "scout_rank75_gcr_exhaustion_veto_15m"
SITE_DIR = ROOT / "reports" / "site" / "factors" / "scout_rank75_gcr_exhaustion_veto_15m"
READING_PATH = ROOT / "reports" / "site" / "reading" / "repo_scout" / "rank75_gcr_exhaustion_veto_clean_replication.html"
...
PRIMARY_VARIANT = "full_gcr_with_divergence"
PRIMARY_COST = 6.0
COSTS = [6.0, 10.0, 15.0]
HOLD_BARS = 8
EARLY_FAIL_BARS = 4
...
def compute_rsi(close: pd.Series, length: int = RSI_LEN) -> pd.Series:
    delta = close.diff()
...
def compute_stoch(df: pd.DataFrame, length: int = STOCH_LEN) -> pd.Series:
...
def compute_divergence_proxy(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    price_pivot_low = out["low"].rolling(PIVOT_WINDOW, min_periods=PIVOT_WINDOW).min().shift(1)
    price_pivot_high = out["high"].rolling(PIVOT_WINDOW, min_periods=PIVOT_WINDOW).max().shift(1)
    rsi_pivot_low = out["rsi14"].rolling(PIVOT_WINDOW, min_periods=PIVOT_WINDOW).min().shift(1)
    rsi_pivot_high = out["rsi14"].rolling(PIVOT_WINDOW, min_periods=PIVOT_WINDOW).max().shift(1)
    out["bullish_divergence_proxy"] = ((out["low"] < price_pivot_low) & (out["rsi14"] > rsi_pivot_low)).fillna(False)
    out["bearish_divergence_proxy"] = ((out["high"] > price_pivot_high) & (out["rsi14"] < rsi_pivot_high)).fillna(False)
    return out
...
def build_frame(asset: str, symbol: str) -> pd.DataFrame:
    ...
    df["stoch14"] = compute_stoch(df)
    bb_mid = df["close"].rolling(BB_LEN, min_periods=BB_LEN).mean()
    bb_std = df["close"].rolling(BB_LEN, min_periods=BB_LEN).std(ddof=0)
    df["bb_upper"] = bb_mid + BB_STD * bb_std
    df["bb_lower"] = bb_mid - BB_STD * bb_std
    df["rvol20"] = df["volume"] / df["vol_ma20"]
    df["exhaustion_volume"] = (df["volume"] > 2.5 * df["vol_ma20"]).fillna(False)
    df = compute_divergence_proxy(df)
...
def exhaustion_flags(frame: pd.DataFrame, setup: str) -> pd.DataFrame:
    direction = direction_for_setup(setup)
    if direction > 0:
        extreme = ((frame["rsi14"] >= 75.0) | (frame["stoch14"] >= 80.0)).fillna(False)
        bb_edge = (frame["high"] >= frame["bb_upper"]).fillna(False)
        divergence = frame["bearish_divergence_proxy"].fillna(False)
    else:
        extreme = ((frame["rsi14"] <= 25.0) | (frame["stoch14"] <= 20.0)).fillna(False)
        bb_edge = (frame["low"] <= frame["bb_lower"]).fillna(False)
        divergence = frame["bullish_divergence_proxy"].fillna(False)
    volume_spike = (frame["rvol20"] > 2.0).fillna(False)
    exhaustion_volume = frame["exhaustion_volume"].fillna(False)
    return pd.DataFrame({
        "extreme": extreme,
        "volume_spike": volume_spike,
        "bb_edge": bb_edge,
        "divergence": divergence,
        "exhaustion_volume": exhaustion_volume,
    }, index=frame.index)


def gate_pass(frame: pd.DataFrame, setup: str, variant: str) -> pd.Series:
    flags = exhaustion_flags(frame, setup)
    veto_extreme = flags["extreme"]
    veto_extreme_volume = flags["extreme"] & flags["volume_spike"]
    veto_extreme_volume_bb = veto_extreme_volume & flags["bb_edge"]
    veto_full = veto_extreme_volume_bb & (flags["exhaustion_volume"] | flags["divergence"])
    if variant == "baseline":
        return pd.Series(True, index=frame.index)
    if variant == "extreme_only":
        return ~veto_extreme
    if variant == "extreme_plus_volume":
        return ~veto_extreme_volume
    if variant == "extreme_plus_volume_plus_bb":
        return ~veto_extreme_volume_bb
    if variant == "full_gcr_with_divergence":
        return ~veto_full
...
def build_signals(frame: pd.DataFrame, asset: str, setup: str, variant: str) -> pd.DataFrame:
    ...
    last_exit = -1
    direction = direction_for_setup(setup)
    for idx in range(max(BB_LEN + 5, 40), len(frame) - HOLD_BARS - 1):
        ...
        rows.append({
            ...
            "stoch14": float(frame.iloc[idx]["stoch14"]),
            "rvol20": float(frame.iloc[idx]["rvol20"]),
            "extreme_flag": bool(flags.iloc[idx]["extreme"]),
            "volume_spike_flag": bool(flags.iloc[idx]["volume_spike"]),
            "bb_edge_flag": bool(flags.iloc[idx]["bb_edge"]),
            "divergence_flag": bool(flags.iloc[idx]["divergence"]),
            "exhaustion_volume_flag": bool(flags.iloc[idx]["exhaustion_volume"]),
...
def build_trades(frame: pd.DataFrame, signals: pd.DataFrame, cost_bps: float) -> pd.DataFrame:
    ...
    for _, sig in signals.iterrows():
        ...
        if sig["setup"] == "breakout_short":
            ...
            adverse = ((probe["high"] / entry_px) - 1.0).max() if len(probe) else np.nan
            ...
            adverse = (1.0 - (probe["low"] / entry_px)).max() if len(probe) else np.nan
        else:
            ema = float(sig["ema15"])
        ...
        rows.append({
            ...
            "mae_4bars": float(max(adverse, 0.0)) if not pd.isna(adverse) else np.nan,
            "rsi14": float(sig["rsi14"]),
            "stoch14": float(sig["stoch14"]),
            "rvol20": float(sig["rvol20"]),
            "extreme_flag": bool(sig["extreme_flag"]),
            "volume_spike_flag": bool(sig["volume_spike_flag"]),
            "bb_edge_flag": bool(sig["bb_edge_flag"]),
            "divergence_flag": bool(sig["divergence_flag"]),
        })
    return pd.DataFrame(rows)
...
def summarize_asset(trades: pd.DataFrame, *, asset: str, setup: str, variant: str, cost_bps: float, signal_events: int) -> dict[str, object]:
    ...
    return {
        ...
        "mae_4bars": float(trades["mae_4bars"].mean()),
    }
...
def verdict_and_notes(overall: pd.DataFrame) -> tuple[str, list[str]]:
    ...
    if primary.empty or baseline.empty:
        return "hard verdict：缺少 Rank 75 主读法结果。", ["主读法 `full_gcr_with_divergence @ 6bps` 或 baseline 对照未产出。"]
    merged = primary.merge(
        baseline[["setup", "mean_total_return", "mean_false_break_ratio", "mean_early_fail_4bars_rate", "mean_trades", "mean_mae_4bars"]],
        on="setup",
        suffixes=("", "_baseline"),
    )
    notes = []
    ...
    okay_retention = int(((merged["mean_trade_count_retention"] >= 0.45) & (merged["mean_trade_count_retention"] <= 0.95)).sum())
    for _, row in merged.iterrows():
        notes.append(
            f"`{row['setup']} / {PRIMARY_VARIANT} @ 6bps`：return≈{pct(row['mean_total_return'])}、retention≈{pct(row['mean_trade_count_retention'])}、false_break≈{pct(row['mean_false_break_ratio'])}、early_fail≈{pct(row['mean_early_fail_4bars_rate'])}、mae4≈{pct(row['mean_mae_4bars'])}；对照 baseline≈{pct(row['mean_total_return_baseline'])}/{pct(row['mean_false_break_ratio_baseline'])}/{pct(row['mean_early_fail_4bars_rate_baseline'])}/{pct(row['mean_mae_4bars_baseline'])}。"
        )
    if better_return >= 2 and (better_false + better_fail) >= 4 and okay_retention >= 2 and float(primary["positive_asset_ratio"].mean()) >= 0.5:
        return "hard verdict：Rank 75 / GCR extreme-sentiment exhaustion veto 在这手最小 clean replication 下还保留了继续 cheap-check 的资格，更像 P1 weak candidate，而不是立刻 park。", notes
    return "hard verdict：Rank 75 / GCR extreme-sentiment exhaustion veto 在当前最小 clean replication 下仍更像 `park / evidence pool`；改善若有，也主要来自局部 setup 或砍单，不够诚实。", notes
...
<head>
  ...
  <title>Rank 75 · GCR exhaustion veto clean replication</title>
  <style>{CSS}</style>
</head>
<body>
  <p><a href='../reading/repo_scout/rank75_gcr_exhaustion_veto_source_intake.html'>← 返回 Rank 75 source intake</a></p>
  <h1>Rank 75 · GCR extreme-sentiment exhaustion veto（minimal clean replication）</h1>
  <p class='muted'>生成时间：{escape(generated_at)}｜固定复用 BTC/ETH/SOL 120d 15m cache；本轮先用同频 15m exhaustion proxy 做最小便宜检查，统一冻结为 <code>signal 当根及之前数据 + next-bar open + no-overlap + hold 8 bars</code>。</p>

  <div class='card'>
    <h2>这轮只回答一个问题</h2>
    <p>当 <code>EMA = waiting_not_due</code> 时，把 <code>不要追最后一脚</code> 的 GCR exhaustion veto 接到 <code>ema_psar_long / fib_retest_long / breakout_short</code> 三条 archetype 上，能不能在不过度砍单的前提下，减少假突破、早死和入场后立刻反抽？</p>
    <p><span class='pill'>只做 clean replication，不扩成新的 reversal / divergence 大研究</span></p>
  </div>
  ...
  <div class='card'>
    ...
    <ul>
      ...
      <li>五臂对照：<code>baseline</code>、<code>extreme_only</code>、<code>extreme_plus_volume</code>、<code>extreme_plus_volume_plus_bb</code>、<code>full_gcr_with_divergence</code>。</li>
      <li><code>extreme_only</code>：只用 RSI/Stoch 极端，不让极端尾段继续开仓。</li>
      <li><code>extreme_plus_volume</code>：极端 + <code>RVOL &gt; 2.0</code>。</li>
      <li><code>extreme_plus_volume_plus_bb</code>：再加 <code>BB edge</code>。</li>
      <li><code>full_gcr_with_divergence</code>：再加 <code>exhaustion volume &gt; 2.5x MA20</code> 或 divergence proxy。</li>
      <li>由于本轮严格复用现有 15m cache，尚未追补 5m veto 层；因此这里是 <b>同频 15m exhaustion proxy</b>，只回答这条线值不值得继续，不能把结果伪装成完整 5m execution-layer 结论。</li>
    </ul>
  </div>
  ...
  <div class='card'>
    ...
    {render_table(overall6, percent_cols={'mean_total_return','positive_asset_ratio','mean_trade_count_retention','mean_avg_net_ret','mean_win_rate','mean_false_break_ratio','mean_early_fail_4bars_rate','mean_forward_4bars','mean_forward_8bars','mean_mae_4bars'}, digits_cols={'cost_bps_per_side':0,'mean_trades':1})}
  </div>

  <div class='card'>
    <h2>asset-level（6bps）</h2>
    {render_table(asset6, percent_cols={'total_return','trade_count_retention','avg_net_ret','win_rate','false_break_ratio','early_fail_4bars_rate','forward_4bars','forward_8bars','mae_4bars'}, digits_cols={'cost_bps_per_side':0,'signal_events':0,'trades':0})}
  </div>

  <div class='card'>
    <h2>time-pocket honesty（主读法 = full_gcr_with_divergence）</h2>
...
def build_reading_html(generated_at: str, verdict: str) -> str:
    return f"""<!doctype html>
<html><head><meta charset='utf-8'><title>Rank 75 clean replication</title><style>{CSS}</style></head><body>
<p><a href='rank75_gcr_exhaustion_veto_source_intake.html'>← 返回 Rank 75 source intake</a></p>
<h1>Rank 75 · minimal clean replication</h1>
<div class='card'>
  <p><span class='pill'>更新时间：{escape(generated_at)}</span><span class='pill'>verdict：{escape(verdict)}</span></p>
  <p>这轮把 Rank 75 的最小 clean replication 跑完了：只比较 <code>baseline / extreme_only / extreme_plus_volume / extreme_plus_volume_plus_bb / full_gcr_with_divergence</code> 五臂，固定 <code>BTC/ETH/SOL 120d 15m</code> cache，统一 <code>next-bar open + no-overlap + hold 8 bars</code>。</p>
  <p>Reader-facing 主落点：<a href='../../factors/scout_rank75_gcr_exhaustion_veto_15m/report.html'>scout_rank75_gcr_exhaustion_veto_15m/report.html</a></p>
</div>
</body></html>"""
...
def update_todo(overall: pd.DataFrame, pockets: pd.DataFrame, generated_at: str, verdict: str) -> None:
    ...
    if 'P1 weak candidate' in verdict:
        queue_line = "**`Run 1 = EMA due-check only（当前最近 due 点仍是 A股 07:00 UTC；若仍 waiting_not_due，不得空转）` -> `Run 2 = 若 Rank 75 仍保留 candidate 资格，则只允许给它 1 个真正会改变 verdict 的最小 Light Stability Pack（默认优先时间稳定性）` -> `Run 3 = 若 Rank 75 cheap check 后仍不能升格，则切到 Rank 76 / intraday clock polarity + event blackout gate 做 fresh source intake；只有 fresh source 这一层也 exhausted 时，才允许回退到 one-regime-per-session overlay > Rank 35b > Rank 16b > tiny-live plumbing`**"
    else:
        queue_line = "**`Run 1 = EMA due-check only（当前最近 due 点仍是 A股 07:00 UTC；若仍 waiting_not_due，不得空转）` -> `Run 2 = 若 Rank 75 minimal clean replication 已给出 park / evidence pool hard verdict，则立刻切到 Rank 76 / intraday clock polarity + event blackout gate 做 fresh source intake` -> `Run 3 = 只有 fresh source 这一层也 exhausted 时，才允许回退到 one-regime-per-session overlay > Rank 35b > Rank 16b > tiny-live plumbing`**"
    insert = (
        f"- **最新补充（{generated_at}）**：这轮再次先核对 `Run 1 / EMA due-check` 与 `P3` 托管位状态：最新 `reports/artifacts/ema_psar_raw_alpha/ema_paper_trading_due_guardrail_snapshot.csv` 仍显示全 desk 无 `due-now / overdue` lane，最早 due 点仍是 `A股三条 lane -> 2026-03-19 07:00 UTC`；`reports/artifacts/manual_narrow_paper_lanes/manual_narrow_paper_last_run_summary.json` 继续是 `new_closed_trades_appended={p3_appends}`。因此本轮合法主动作仍是 **`Run 2 / Rank 75 minimal clean replication`**，而不是继续回头挤占 `P3 continuity`。\n"
        f"  - 这轮已把 **`Rank 75 / GCR extreme-sentiment exhaustion veto`** 的唯一那手最小 clean replication 跑完：固定复用 `BTC/ETH/SOL 120d 15m` cache，只接到 `ema_psar_long / fib_retest_long / breakout_short` 三条 archetype，上比较 `baseline / extreme_only / extreme_plus_volume / extreme_plus_volume_plus_bb / full_gcr_with_divergence` 五臂，并统一冻结到 **`signal 当根及之前数据 + next-bar open + no-overlap + hold 8 bars`**。\n"
        f"  - `6bps/side` 下，主读法 `full_gcr_with_divergence` 的结果为：`ema_psar_long≈{pct(primary.loc['ema_psar_long','mean_total_return'])} / retention≈{pct(primary.loc['ema_psar_long','mean_trade_count_retention'])} / false_break≈{pct(primary.loc['ema_psar_long','mean_false_break_ratio'])}`；`fib_retest_long≈{pct(primary.loc['fib_retest_long','mean_total_return'])} / retention≈{pct(primary.loc['fib_retest_long','mean_trade_count_retention'])} / false_break≈{pct(primary.loc['fib_retest_long','mean_false_break_ratio'])}`；`breakout_short≈{pct(primary.loc['breakout_short','mean_total_return'])} / retention≈{pct(primary.loc['breakout_short','mean_trade_count_retention'])} / false_break≈{pct(primary.loc['breakout_short','mean_false_break_ratio'])}`。对照 baseline：`ema_psar_long≈{pct(baseline.loc['ema_psar_long','mean_total_return'])}`、`fib_retest_long≈{pct(baseline.loc['fib_retest_long','mean_total_return'])}`、`breakout_short≈{pct(baseline.loc['breakout_short','mean_total_return'])}`；breakout_short 的 time-pocket：{pocket_text}。\n"
        f"  - 由于本轮严格复用现有 cache，没有额外追补 `5m` veto 层；因此这是 **同频 15m exhaustion proxy** 的便宜诚实检查，不是完整 execution-layer 终局证据。\n"
        f"  - 因此当前更诚实的 hard verdict 是 **`{verdict}`**。\n"
        f"  - 网页落点：`reports/site/factors/scout_rank75_gcr_exhaustion_veto_15m/report.html`、`reports/site/reading/repo_scout/rank75_gcr_exhaustion_veto_clean_replication.html`；artifact：`reports/artifacts/scout_rank75_gcr_exhaustion_veto_15m/overall_summary.csv`、`asset_summary.csv`、`time_pocket_summary.csv`。\n"
...
def main() -> None:
    ...
    for asset, symbol in ASSETS.items():
        ...
        for setup in SETUPS:
            ...
            for variant in VARIANTS:
                ...
                (signals if not signals.empty else pd.DataFrame(columns=['signal_id','asset','setup','variant','direction','signal_idx','entry_idx','signal_ts','signal_price','rsi14','stoch14','rvol20','extreme_flag','volume_spike_flag','bb_edge_flag','divergence_flag','exhaustion_volume_flag','ema15','breakout_anchor','fib_618']))\
    ...
    print(generated_at)
    print(verdict)
    print(overall[(overall['cost_bps_per_side'] == PRIMARY_COST)][['setup','variant','mean_total_return','positive_asset_ratio','mean_trades','mean_trade_count_retention','mean_false_break_ratio','mean_early_fail_4bars_rate','mean_mae_4bars']].to_string(index=False))
