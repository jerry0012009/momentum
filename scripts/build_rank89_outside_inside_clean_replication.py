...
ART_DIR = ROOT / "reports" / "artifacts" / "scout_rank89_outside_close_back_inside_failure_15m"
SITE_DIR = ROOT / "reports" / "site" / "factors" / "scout_rank89_outside_close_back_inside_failure_15m"
READING_PATH = ROOT / "reports" / "site" / "reading" / "repo_scout" / "rank89_outside_close_back_inside_failure_clean_replication.html"

ASSETS = {"BTC-USD": "BTCUSDT", "ETH-USD": "ETHUSDT", "SOL-USD": "SOLUSDT"}
SETUPS = ["ema_psar_long", "fib_retest_long", "breakout_short"]
VARIANTS = ["baseline", "outside_inside_binary", "outside_inside_seqext_size"]
PRIMARY_VARIANT = "outside_inside_binary"
PRIMARY_COST = 6.0
COSTS = [6.0, 10.0, 15.0]
HOLD_BARS = 8
EARLY_FAIL_BARS = 4
RANGE_BARS = 16
RETURN_TO_INSIDE_MAX_BARS = 4
VERDICT_LOOKBACK_BARS = 4
EPS = 1e-12
CSS = """
...
def build_failure_overlay(df: pd.DataFrame) -> pd.DataFrame:
    zone_high = df["high"].rolling(RANGE_BARS, min_periods=RANGE_BARS).max().shift(1)
    zone_low = df["low"].rolling(RANGE_BARS, min_periods=RANGE_BARS).min().shift(1)
    zone_width = (zone_high - zone_low).clip(lower=EPS)

    long_flag = np.zeros(len(df), dtype=bool)
    short_flag = np.zeros(len(df), dtype=bool)
    overshoot = np.full(len(df), np.nan)
    seq_extreme = np.full(len(df), np.nan)
    seq_len = np.full(len(df), np.nan)

    pending_top: list[dict] = []
    pending_bottom: list[dict] = []
    ...
    for i in range(len(df)):
        ...
        zl = zone_low.iat[i]
        zw = zone_width.iat[i]
        if pd.isna(zh) or pd.isna(zl) or pd.isna(zw):
            continue

        new_top = []
        for ev in pending_top:
            bars_since = i - ev["idx"]
            if bars_since > RETURN_TO_INSIDE_MAX_BARS:
                continue
            ev["seq_high"] = max(ev["seq_high"], high[i])
            if zl <= close[i] <= zh:
                short_flag[i] = True
                overshoot[i] = max(overshoot[i], (ev["seq_high"] - zh) / max(zw, EPS)) if not np.isnan(overshoot[i]) else (ev["seq_high"] - zh) / max(zw, EPS)
                seq_extreme[i] = ev["seq_high"]
                seq_len[i] = bars_since
            else:
                new_top.append(ev)
        pending_top = new_top

        new_bottom = []
        for ev in pending_bottom:
            bars_since = i - ev["idx"]
            if bars_since > RETURN_TO_INSIDE_MAX_BARS:
                continue
            ev["seq_low"] = min(ev["seq_low"], low[i])
            if zl <= close[i] <= zh:
                long_flag[i] = True
                overshoot[i] = max(overshoot[i], (zl - ev["seq_low"]) / max(zw, EPS)) if not np.isnan(overshoot[i]) else (zl - ev["seq_low"]) / max(zw, EPS)
                seq_extreme[i] = ev["seq_low"]
                seq_len[i] = bars_since
            else:
                new_bottom.append(ev)
        pending_bottom = new_bottom

        if close[i] > zh:
            pending_top.append({"idx": i, "seq_high": high[i]})
        elif close[i] < zl:
            pending_bottom.append({"idx": i, "seq_low": low[i]})

    out = df.copy()
    out["zone_high_16"] = zone_high
    out["zone_low_16"] = zone_low
    out["zone_width_16"] = zone_width
    out["failure_long_event"] = long_flag
    out["failure_short_event"] = short_flag
    out["failure_overshoot"] = pd.Series(overshoot).clip(lower=0)
    out["failure_seq_extreme"] = seq_extreme
    out["failure_seq_len"] = seq_len
    out["recent_long_failure"] = pd.Series(long_flag).rolling(VERDICT_LOOKBACK_BARS, min_periods=1).max().shift(1).fillna(0).astype(bool)
    out["recent_short_failure"] = pd.Series(short_flag).rolling(VERDICT_LOOKBACK_BARS, min_periods=1).max().shift(1).fillna(0).astype(bool)
    out["recent_failure_overshoot"] = out["failure_overshoot"].rolling(VERDICT_LOOKBACK_BARS, min_periods=1).max().shift(1)
    return out
...
def build_frame(asset: str, symbol: str) -> pd.DataFrame:
    ...
    return build_failure_overlay(df)


def variant_policy(row: pd.Series, setup: str, variant: str) -> tuple[bool, float, str]:
    if variant == "baseline":
        return True, 1.0, "baseline"
    wants_long = setup.endswith("long")
    flag = bool(row["recent_long_failure"] if wants_long else row["recent_short_failure"])
    if not flag:
        return False, 1.0, "no_matching_failure_verdict"
    if variant == "outside_inside_binary":
        return True, 1.0, "matching_failure_verdict"
    overshoot = float(row["recent_failure_overshoot"]) if pd.notna(row["recent_failure_overshoot"]) else 0.0
    size_mult = 0.5 if overshoot >= 0.25 else 1.0
    return True, size_mult, f"matching_failure_verdict_overshoot_{'deep' if size_mult < 1.0 else 'mild'}"
...
def build_trades(frame: pd.DataFrame, asset: str, setup: str, variant: str) -> tuple[pd.DataFrame, int]:
    ...
    for idx in range(max(60, RANGE_BARS + RETURN_TO_INSIDE_MAX_BARS + 2), len(frame) - HOLD_BARS - 2):
        ...
        rows.append(
            {
                ...
                "recent_failure_overshoot": row.get("recent_failure_overshoot"),
                "recent_long_failure": bool(row.get("recent_long_failure", False)),
                "recent_short_failure": bool(row.get("recent_short_failure", False)),
            }
        )
            ...
            overall_rows.append(
                {
                    ...
                    "mean_recent_overshoot": var_df.groupby("asset")["recent_failure_overshoot"].mean().mean() if not var_df.empty else np.nan,
                }
            )
            for setup in SETUPS:
...
def decide_verdict(overall: pd.DataFrame, setup_summary: pd.DataFrame) -> tuple[str, str, str]:
    ...
    if p_ret > 0 and p_pos >= 2/3 and p_retention >= 0.40 and p_fail <= b_fail * 0.92:
        return "promote_to_P2 / paper-candidate pool", (
            f"{best_variant} 在 6bps 下不是单纯靠砍样本少亏：成本后均值转正、跨资产至少 2/3 为正、retention 仍 >= 40%，且 4-bar early-fail 明显优于 baseline，足够先升到 P2。"
        ), best_variant
    if p_ret > b_ret + 0.005 and p_retention >= 0.18 and (p_fail < b_fail or best_setup > 0):
        return "keep_P1 / mixed but honest", (
            f"{best_variant} 相比 baseline 有一定去坏 break / 去亏损改善，但 retention 还偏薄、跨资产也不够硬，先留在 P1 evidence_pool，不直接升到 P2。"
        ), best_variant
    return "park / evidence_pool", (
        f"{best_variant} 当前主要还是靠缩样本或局部 setup 少亏，trade_count_retention / 跨资产一致性都不够，仍不足以诚实占住 fast-lane。"
    ), best_variant
...
def main() -> None:
    ...
    meta = pd.DataFrame([
        {
            ...
            "return_to_inside_max_bars": RETURN_TO_INSIDE_MAX_BARS,
            "verdict_lookback_bars": VERDICT_LOOKBACK_BARS,
            "variants": "baseline / outside_inside_binary / outside_inside_seqext_size",
    ...
    baseline = overall[(overall["variant"] == "baseline") & (overall["cost_bps"] == PRIMARY_COST)].iloc[0]
    binary = overall[(overall["variant"] == "outside_inside_binary") & (overall["cost_bps"] == PRIMARY_COST)].iloc[0]
    seq = overall[(overall["variant"] == "outside_inside_seqext_size") & (overall["cost_bps"] == PRIMARY_COST)].iloc[0]

    body = f"""
<h1>Rank 89 / outside-close → back-inside-close failure verdict</h1>
...
<div class='card'>
  ...
  <ul>
    ...
    <li>outside→inside binary：收益 {escape(pct(binary['mean_total_return']))}，资产为正占比 {escape(pct(binary['positive_asset_ratio']))}，retention {escape(pct(binary['trade_count_retention']))}，4bar early-fail {escape(pct(binary['mean_early_fail_4bars']))}</li>
    <li>binary + sequence-extreme sizing：收益 {escape(pct(seq['mean_total_return']))}，资产为正占比 {escape(pct(seq['positive_asset_ratio']))}，retention {escape(pct(seq['trade_count_retention']))}，平均仓位 {escape(pct(seq['mean_position_size_mult']))}，4bar early-fail {escape(pct(seq['mean_early_fail_4bars']))}</li>
  </ul>
</div>
<div class='card'>
  ...
  <ul>
    ...
    <li>rolling 区间：前 <code>16</code> 根 15m 的高低区间（约 4 小时），全部 <code>shift(1)</code>，避免同 bar 偷看。</li>
    <li>outside close：<code>close &gt; zone_high</code> 或 <code>close &lt; zone_low</code>。</li>
    <li>back-inside close：在接下来 <code>1~4</code> 根内，收盘回到区间内部。</li>
    <li>binary verdict：只有最近 <code>4</code> 根内出现匹配方向的 failure verdict，才放行对应 base setup。</li>
    <li>sequence-extreme sizing：若最近一次 overshoot 深度 <code>&gt;= 25%</code> 的区间宽度，则仓位降到 <code>0.5x</code>；否则维持 <code>1.0x</code>。</li>
  </ul>
</div>
<div class='card'>
  <h2>Overall summary</h2>
  {render_table(overall, percent_cols={'mean_total_return','positive_asset_ratio','trade_count_retention','mean_position_size_mult','mean_early_fail_4bars','mean_recent_overshoot'})}
</div>
<div class='card'>
  <h2>Setup summary</h2>
...
<div class='card'>
  <h2>Artifacts</h2>
  <ul>
    <li><code>reports/artifacts/scout_rank89_outside_close_back_inside_failure_15m/overall_summary.csv</code></li>
    <li><code>reports/artifacts/scout_rank89_outside_close_back_inside_failure_15m/setup_summary.csv</code></li>
    <li><code>reports/artifacts/scout_rank89_outside_close_back_inside_failure_15m/asset_summary.csv</code></li>
    <li><code>reports/artifacts/scout_rank89_outside_close_back_inside_failure_15m/trade_samples.csv</code></li>
    <li><code>reports/artifacts/scout_rank89_outside_close_back_inside_failure_15m/meta.csv</code></li>
  </ul>
</div>
"""
    write_html(SITE_DIR / "report.html", "Rank 89 outside-close back-inside clean replication", body)
    write_html(READING_PATH, "Rank 89 outside-close back-inside clean replication", body)
