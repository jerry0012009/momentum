...
PRIMARY_VARIANT = "averaged_zone_context_gate"
PARAM_CONFIGS = [
...
def latest_single_high(pivots: list[dict], t: int) -> dict | None:
...
def prepare_bars(asset: str, symbol: str, *, cluster_atr: float = ZONE_CLUSTER_ATR, zone_width_max_atr: float = ZONE_WIDTH_MAX_ATR) -> pd.DataFrame:
    ...
    zone_break_sig = np.zeros(n, dtype=bool)
    zone_retest_sig = np.zeros(n, dtype=bool)
    context_sig = np.zeros(n, dtype=bool)

    breakout_memory: list[dict] = []

    for t in range(n):
        atr_here = float(bars.iloc[t]["atr"]) if pd.notna(bars.iloc[t]["atr"]) else float("nan")
        latest = latest_single_high(pivots, t)
        if latest is not None:
            single_high[t] = float(latest["price"])
        ...
        if zone is not None:
            ...
            zone_width_ok[t] = bool(math.isfinite(zone["zone_width"]) and math.isfinite(atr_here) and atr_here > 0 and zone["zone_width"] <= zone_width_max_atr * atr_here)

        row = bars.iloc[t]
        if not (bool(row["ema_long"]) and math.isfinite(atr_here) and atr_here > 0):
            continue

        if math.isfinite(zone_high[t]) and int(zone_count[t]) >= 2 and float(row["close"]) > float(zone_high[t]) + BREAK_BUFFER_ATR * atr_here:
            zone_break_sig[t] = True
            breakout_memory.append(
                {
                    "breakout_idx": t,
                    "expires": min(t + RETEST_LOOKAHEAD_BARS, n - 1),
                    "zone_high": float(zone_high[t]),
                    "zone_mid": float(zone_mid[t]),
                    "zone_width_ok": bool(zone_width_ok[t]),
                }
            )

        keep_memory: list[dict] = []
        for info in breakout_memory:
            if t <= int(info["breakout_idx"]):
                keep_memory.append(info)
                continue
            if t > int(info["expires"]):
                continue
            touch = float(row["low"]) <= float(info["zone_high"]) + RETEST_TOUCH_ATR * atr_here
            hold = float(row["close"]) >= float(info["zone_high"]) + RETEST_BUFFER_ATR * atr_here
            not_lost_mid = float(row["close"]) >= float(info["zone_mid"])
            if touch and hold and not_lost_mid:
                zone_retest_sig[t] = True
                if bool(row["ema_long"]) and bool(info["zone_width_ok"]):
                    context_sig[t] = True
            keep_memory.append(info)
        breakout_memory = keep_memory

    bars["single_high"] = single_high
    ...
    bars["single_line_break"] = (
        bars["single_high"].notna()
        & bars["ema_long"]
        & (bars["close"] > bars["single_high"] + BREAK_BUFFER_ATR * bars["atr"])
    ).fillna(False)
    bars["averaged_zone_break"] = pd.Series(zone_break_sig, index=bars.index)
    bars["averaged_zone_retest"] = pd.Series(zone_retest_sig, index=bars.index)
    bars["averaged_zone_context_gate"] = pd.Series(context_sig, index=bars.index)
    return bars
...
def build_events(bars: pd.DataFrame, variant: str) -> pd.DataFrame:
    ...
    for idx in np.where(transition)[0]:
        ...
        if not math.isfinite(atr) or atr <= 0:
            continue
        trigger_level = float(row["single_high"]) if variant == "single_line_break" else float(row["zone_high"])
        if not math.isfinite(trigger_level):
            continue
        future = bars.iloc[idx + 1 : idx + 4]
        false_break_proxy = float((future["close"] <= trigger_level).any()) if not future.empty else np.nan
        rows.append(
            {
                "asset": row["asset"],
                "variant": variant,
                ...
                "ema_long": int(bool(row["ema_long"])),
                "zone_width_ok": int(bool(row["zone_width_ok"])),
        ...
        trades.append(
            {
                ...
                "zone_count": int(event["zone_count"]),
                "zone_width_atr": float(event["zone_width_atr"]) if pd.notna(event["zone_width_atr"]) else np.nan,
...
def derive_verdict(
    ...
    verdict_tag = "park"
    headline = "hard verdict：averaged support/resistance zone + context gate 这轮更像 `park / evidence pool`，暂不进入 paper candidate pool。"
    if (
        ...
        headline = f"hard verdict：averaged SR zone 候选的 {winner} 已通过最小 clean replication + Light Stability Pack，可进入 `{verdict_tag}`。"
    bullets = []
    ...
    bullets.append("两条轻量诚实守门已通过：规则能明确写成 trade on / trade off；zone 只由已确认 swing highs 构造，不用未来标签或突破后回写 zone。")
    ...
    if verdict_tag == "park":
        bullets.append("因此这条线当前更像 Scout 证据包，不该继续占用 Rank 2 那类 narrow-paper wiring 资源。")
    else:
...
<head>
  ...
  <title>Scout Seat · averaged SR zone + context · clean replication</title>
  <style>
...
<body>
  ...
  <h1>Scout Seat · averaged support/resistance zone + context gate · 15m crypto clean replication</h1>
  ...
  <div class="card">
    ...
    <ul>
      ...
      <li>zone：<code>过去 80 bars 内已确认 swing highs</code>；averaged zone 只用与最新高点相距 <code>0.30 ATR</code> 以内的近邻 high 聚类</li>
      <li>变体：<code>single_line_break / averaged_zone_break / averaged_zone_retest / averaged_zone_context_gate</code></li>
      <li>context：<code>EMA20 > EMA50</code> 且 <code>zone_width <= 1.2 ATR</code></li>
  ...
  <div class="card">
    ...
    {summary_table}
    <p class="muted">artifact：<code>reports/artifacts/scout_sr_zone_context_15m/overall_summary.csv</code></p>
  </div>

  <div class="card">
    <h2>winner per-asset（6bps/side）</h2>
    {asset_table}
    <p class="muted">这里把 <code>candidate_events</code>、<code>trades</code>、<code>no_trade_ratio</code> 与 <code>false_break_proxy</code> 一起外显，避免把“少做交易”或“少假突破”误写成 alpha 增量。</p>
  </div>
  ...
  <div class="card">
    <h2>parameter neighbor grid（context_gate）</h2>
    {grid_table}
    <p class="muted">这里只看 zone cluster / width cap 的小邻域，防止把单点 zone pocket 写成结构 alpha。</p>
  </div>

  <div class="card">
    <h2>怎么读这页</h2>
    <ul>
      <li>如果 zone 版本只是靠 <code>no_trade_ratio</code> 飙升才守住收益，就应该直接 <code>park</code>。</li>
      <li>如果更复杂的 averaged/retest/context 版本并不优于 <code>single_line_break</code>，说明“线换成带 + 再加 context”并没有给出足够增量。</li>
...
def main() -> int:
    ...
    print("[ok] sr zone context clean replication generated")
