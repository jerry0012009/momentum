...
ART_DIR = ROOT / "reports" / "artifacts" / "scout_asymmetry_tsmom_15m"
SITE_DIR = ROOT / "reports" / "site" / "factors" / "scout_asymmetry_tsmom_15m"
...
PRIMARY_COST = 6.0
PRIMARY_VARIANT = "pm_guard_100"
PRIMARY_MOM_WINDOW = 16
PRIMARY_PM_WINDOW = 32
VARIANTS = [
    {"variant": "baseline_sign_mom", "guard_ratio": None},
    {"variant": "pm_guard_090", "guard_ratio": 0.90},
    {"variant": "pm_guard_100", "guard_ratio": 1.00},
    {"variant": "pm_guard_110", "guard_ratio": 1.10},
    {"variant": "pm_guard_125", "guard_ratio": 1.25},
]
PARAM_CONFIGS = [
    {"label": "m12_p24_g100", "mom_window": 12, "pm_window": 24, "guard_ratio": 1.00},
    {"label": "m16_p24_g100", "mom_window": 16, "pm_window": 24, "guard_ratio": 1.00},
    {"label": "m16_p32_g100", "mom_window": 16, "pm_window": 32, "guard_ratio": 1.00},
    {"label": "m20_p32_g100", "mom_window": 20, "pm_window": 32, "guard_ratio": 1.00},
    {"label": "m16_p40_g100", "mom_window": 16, "pm_window": 40, "guard_ratio": 1.00},
]
...
def prepare_bars(asset: str, symbol: str, *, mom_window: int, pm_window: int) -> pd.DataFrame:
    bars = load_cached_bars(symbol).copy()
    bars["asset"] = asset
    bars["bar_ret"] = bars["close"].pct_change().fillna(0.0)
    bars["mom"] = bars["close"] / bars["close"].shift(mom_window) - 1.0
    bars["up_sq"] = np.square(np.clip(bars["bar_ret"], 0.0, None))
    bars["down_sq"] = np.square(np.clip(-bars["bar_ret"], 0.0, None))
    bars["up_pm"] = bars["up_sq"].rolling(pm_window, min_periods=pm_window).mean()
    bars["down_pm"] = bars["down_sq"].rolling(pm_window, min_periods=pm_window).mean()
    bars["long_tail_ratio"] = bars["down_pm"] / (bars["up_pm"] + 1e-12)
    bars["short_tail_ratio"] = bars["up_pm"] / (bars["down_pm"] + 1e-12)
    return bars


def build_variant_positions(bars: pd.DataFrame, variant_cfg: dict) -> pd.DataFrame:
    out = bars.copy()
    desired = np.sign(out["mom"]).astype(float)
    guard_ratio = variant_cfg["guard_ratio"]
    if guard_ratio is not None:
        long_ok = (out["mom"] > 0) & (out["long_tail_ratio"] < guard_ratio)
        short_ok = (out["mom"] < 0) & (out["short_tail_ratio"] < guard_ratio)
        desired = np.where(long_ok, 1.0, np.where(short_ok, -1.0, 0.0))
    else:
        desired = np.where(out["mom"] > 0, 1.0, np.where(out["mom"] < 0, -1.0, 0.0))
    desired = pd.Series(desired, index=out.index, dtype=float).where(pd.notna(out["mom"]), 0.0)
...
def simulate_variant(bars: pd.DataFrame, variant_cfg: dict, cost_bps_per_side: float) -> tuple[pd.DataFrame, pd.DataFrame]:
    df = build_variant_positions(bars, variant_cfg)
    ...
    return pd.DataFrame([
        {
            ...
            "mean_abs_position": float(active["position"].abs().mean()) if not active.empty else np.nan,
        }
    ])
...
def build_time_stability(primary_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    rows.append({"gate": "bucket_trade_floor", "status": "pass" if float(summary["mean_trade_events"].min()) >= 40 else "watch", "actual": num(float(summary["mean_trade_events"].min()), 1), "threshold": ">= 40 平均切换", "why_it_matters": "时间切片里仍要有最小交易/切换密度。"})
    return pd.DataFrame(rows), detail


def run_param_config(cfg: dict) -> pd.DataFrame:
    rows = []
    variant_cfg = {"variant": PRIMARY_VARIANT, "guard_ratio": cfg["guard_ratio"]}
    for asset, symbol in ASSETS.items():
        bars = prepare_bars(asset, symbol, mom_window=cfg["mom_window"], pm_window=cfg["pm_window"])
        sim, _ = simulate_variant(bars, variant_cfg, PRIMARY_COST)
        rows.append(summarize_asset_variant(sim, asset, PRIMARY_VARIANT, PRIMARY_COST).iloc[0])
...
def build_parameter_stability() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    gates = pd.DataFrame([
        ...
        {"gate": "trade_density_floor", "status": "pass" if pd.notna(min_trade_events) and min_trade_events >= 80 else "watch", "actual": num(min_trade_events, 1), "threshold": ">= 80 平均切换", "why_it_matters": "参数变化后不能只剩极薄样本。"},
    ])
...
def build_cost_trade_stability(overall_summary: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame([
        ...
        {"gate": "trade_floor", "status": "pass" if pd.notna(min_trades) and min_trades >= 120 else "watch", "actual": num(min_trades, 1), "threshold": ">= 120 平均切换", "why_it_matters": "过滤后不能只剩极少交易。"},
    ])


def build_meta(overall_summary: pd.DataFrame, primary_summary: pd.DataFrame, time_gates: pd.DataFrame, param_gates: pd.DataFrame, cross_gates: pd.DataFrame, cost_gates: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame([
        {
            ...
            "candidate": "partial-moment asymmetry TSMOM gate",
            "source": "Liu, Lu, Wang (2021)",
...
<head>
  <meta charset=\"utf-8\" />
  <title>Scout｜Asymmetry TSMOM 15m</title>
  <style>
...
<body>
  <h1>Scout Seat｜Partial-moment asymmetry TSMOM gate（15m crypto）</h1>
  ...
  <h2>一句话结论</h2>
  <p>这次把 <code>sign(mom)</code> 上再叠一层 <code>partial-moment asymmetry</code> 风险门：多头避开最近下行尾部突然变重、空头避开上行尾部突然变重的 bar。结果是样本内能把持仓密度降下来，但在 BTC/ETH/SOL 的 120d / 15m cache 上，<b>{escape(PRIMARY_VARIANT)}</b> 在 <b>{pct(primary_row['mean_total_return'])}</b>、<b>{num(primary_row['positive_assets'], 0)}/{num(primary_row['assets_tested'], 0)}</b> 资产为正、平均切换 <b>{num(primary_row['mean_trade_events'], 1)}</b> 的读法下，仍不够支撑进入 paper candidate，当前更诚实的结论仍是 <b>park</b>。</p>

  <h2>Clean-room 规则</h2>
  <ul>
    <li><code>mom_16 = close / close.shift(16) - 1</code></li>
    <li><code>up_pm / down_pm</code>：最近 32 根 15m 收益的正负平方收益均值</li>
    <li>baseline：<code>mom &gt; 0</code> 做多，<code>mom &lt; 0</code> 做空</li>
    <li>guard：多头仅在 <code>down_pm / up_pm &lt; threshold</code> 时允许；空头仅在 <code>up_pm / down_pm &lt; threshold</code> 时允许</li>
  ...
  <h2>为什么当前先 park</h2>
  <ul>
    <li>如果这层 asymmetry guard 真有明显价值，至少应在 3 个币种里让 2 个存活，或在多个成本档下不至于整体继续为负；当前没有做到。</li>
    <li>它更像是“减少坏局面暴露”的 risk-aware gate，而不是能单独撑起 15m crypto alpha 的 candidate。</li>
    <li>因此当前更适合把它留在 evidence pool，作为未来 EMA / TSMOM 风险门的参考，而不是继续升格到 paper candidate。</li>
  </ul>
</body>
</html>
"""
...
def main() -> None:
    ...
    SPEC_PATH.write_text(
        "candidate,source,mom_window,pm_window,primary_variant,rule\n"
        "partial-moment asymmetry TSMOM gate,Liu et al. 2021,16,32,pm_guard_100,sign(momentum) plus asymmetric partial-moment risk gate\n",
        encoding="utf-8",
    )
    ...
    for asset, symbol in ASSETS.items():
        bars = prepare_bars(asset, symbol, mom_window=PRIMARY_MOM_WINDOW, pm_window=PRIMARY_PM_WINDOW)
        for variant_cfg in VARIANTS:
            for cost in COSTS:
                sim, nav = simulate_variant(bars, variant_cfg, cost)
                sim_frames.append(sim)
                nav_frames.append(nav)
                asset_summary_frames.append(summarize_asset_variant(sim, asset, variant_cfg["variant"], cost))
    ...
    meta = build_meta(overall_summary, primary_summary, time_gates, param_gates, cross_gates, cost_gates)
    ...
    print("[ok] asymmetry tail tsmom clean replication generated")
