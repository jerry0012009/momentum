...
READING_PATH = ROOT / "reports" / "site" / "reading" / "repo_scout" / "rank110_psar_preflip_reclaim_clean_replication.html"
...
VARIANTS = ["baseline", "preflip_reclaim_long_only", "preflip_reclaim_symmetric"]
...
def build_psar_reclaim_state(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    prev_close = out["close"].shift(1)
    prev_psar = out["psar"].shift(1)
    out["bull_flip"] = (prev_close < prev_psar) & (out["close"] > out["psar"])
    out["bear_flip"] = (prev_close > prev_psar) & (out["close"] < out["psar"])

    bull_dot = []
    bear_dot = []
    bull_age = []
    bear_age = []
    last_bull_dot = np.nan
    last_bear_dot = np.nan
    last_bull_idx: int | None = None
    last_bear_idx: int | None = None

    for i, row in out.iterrows():
        if bool(row["bull_flip"]):
            last_bull_dot = float(prev_psar.iloc[i]) if pd.notna(prev_psar.iloc[i]) else np.nan
            last_bull_idx = i
        if bool(row["bear_flip"]):
            last_bear_dot = float(prev_psar.iloc[i]) if pd.notna(prev_psar.iloc[i]) else np.nan
            last_bear_idx = i
        bull_dot.append(last_bull_dot)
        bear_dot.append(last_bear_dot)
        bull_age.append((i - last_bull_idx) if last_bull_idx is not None else np.nan)
        bear_age.append((i - last_bear_idx) if last_bear_idx is not None else np.nan)

    out["pre_bull_flip_dot"] = bull_dot
    out["pre_bear_flip_dot"] = bear_dot
    out["bars_since_bull_flip"] = bull_age
    out["bars_since_bear_flip"] = bear_age

    out["bull_reclaim_signal"] = (
        out["bars_since_bull_flip"].between(1, N_BARS)
        & out["pre_bull_flip_dot"].notna()
        & (out["open"] < out["pre_bull_flip_dot"])
        & (out["close"] > out["pre_bull_flip_dot"])
    ).fillna(False)
    out["bear_reclaim_signal"] = (
        out["bars_since_bear_flip"].between(1, N_BARS)
        & out["pre_bear_flip_dot"].notna()
        & (out["open"] > out["pre_bear_flip_dot"])
        & (out["close"] < out["pre_bear_flip_dot"])
    ).fillna(False)
    return out
...
def build_frame(asset: str, symbol: str) -> pd.DataFrame:
    ...
    return build_psar_reclaim_state(df)
...
def variant_decision(row: pd.Series, setup: str, variant: str) -> tuple[bool, float, str]:
    long_gate = bool(row.get("bull_reclaim_signal", False))
    short_gate = bool(row.get("bear_reclaim_signal", False))
    if variant == "baseline":
        return True, 1.0, "baseline"
    if variant == "preflip_reclaim_long_only":
        if setup in LONG_SETUPS:
            return long_gate, 1.0, "long_reclaim_ok" if long_gate else "long_reclaim_missing"
        return True, 1.0, "short_baseline_unchanged"
    if variant == "preflip_reclaim_symmetric":
        if setup in LONG_SETUPS:
            return long_gate, 1.0, "long_reclaim_ok" if long_gate else "long_reclaim_missing"
        return short_gate, 1.0, "short_reclaim_ok" if short_gate else "short_reclaim_missing"
        ...
        rows.append(
            {
                ...
                "bull_reclaim_signal": bool(row.get("bull_reclaim_signal", False)),
                "bear_reclaim_signal": bool(row.get("bear_reclaim_signal", False)),
                "pre_bull_flip_dot": float(row["pre_bull_flip_dot"]) if pd.notna(row["pre_bull_flip_dot"]) else np.nan,
                "pre_bear_flip_dot": float(row["pre_bear_flip_dot"]) if pd.notna(row["pre_bear_flip_dot"]) else np.nan,
                "bars_since_bull_flip": float(row["bars_since_bull_flip"]) if pd.notna(row["bars_since_bull_flip"]) else np.nan,
                "bars_since_bear_flip": float(row["bars_since_bear_flip"]) if pd.notna(row["bars_since_bear_flip"]) else np.nan,
            }
        )
...
def build_verdict(overall_summary: pd.DataFrame, setup_summary: pd.DataFrame) -> tuple[str, str]:
    ...
    symmetric = overall_summary.loc[overall_summary["variant"] == "preflip_reclaim_symmetric"].iloc[0]
    setup_base = setup_summary.set_index(["variant", "setup"])
    ema_base = setup_base.loc[("baseline", "ema_psar_long")]
    ema_long = setup_base.loc[(PRIMARY_VARIANT, "ema_psar_long")] if (PRIMARY_VARIANT, "ema_psar_long") in setup_base.index else None
    fib_base = setup_base.loc[("baseline", "fib_retest_long")]
    fib_long = setup_base.loc[(PRIMARY_VARIANT, "fib_retest_long")] if (PRIMARY_VARIANT, "fib_retest_long") in setup_base.index else None
    short_sym = setup_base.loc[("preflip_reclaim_symmetric", "breakout_short")] if ("preflip_reclaim_symmetric", "breakout_short") in setup_base.index else None

    improve_long = (
        long_only["mean_total_return"] > base["mean_total_return"]
        and long_only["false_follow_through_4bars"] <= base["false_follow_through_4bars"]
    )
    short_bad = bool(short_sym is not None and short_sym["total_return"] < ema_base["total_return"])
    long_retention_ok = long_only["trade_count_retention"] >= 0.35
    long_setup_ok = bool(
        ema_long is not None
        and ema_long["total_return"] > ema_base["total_return"]
        and fib_long is not None
        and fib_long["total_return"] >= fib_base["total_return"]
    )

    if improve_long and long_retention_ok and symmetric["mean_total_return"] < long_only["mean_total_return"]:
        return (
            "keep_P1_as_asymmetric_long_filter",
            "long-only 版本比 baseline 更诚实，但一旦强行镜像到 short 侧就明显变坏；因此这条线当前更像 long-side optional filter / continuation admission note，不足以升 P2，也不该包装成 shared gate。",
        )
    if long_only["trade_count_retention"] < 0.20 or long_only["mean_total_return"] <= base["mean_total_return"]:
        return (
            "park",
            "pre-flip reclaim 主要靠砍样本，连 long-only aggregate 都没有比 baseline 更诚实；当前应直接压回 park / evidence pool。",
        )
    return (
        "keep_P1_mixed",
        "pre-flip reclaim 带来一点 long-side 过滤味道，但跨 setup / 跨资产 / short 镜像都不够硬；当前只够留在 P1 evidence pool，不足以升 P2。",
    )
...
def main() -> None:
    ...
    sym_row = overall_summary.loc[overall_summary["variant"] == "preflip_reclaim_symmetric"].iloc[0]

    report_body = f"""
<h1>Rank 110 / PSAR pre-flip SAR dot reclaim gate — clean replication</h1>
<div class='card'>
  <p><strong>结论：</strong><span class='{'good' if 'park' not in verdict else 'bad'}'>{escape(verdict)}</span></p>
  <p>{escape(verdict_note)}</p>
  <p class='muted'>最小复现实验：固定 BTC/ETH/SOL 120d 15m cache，统一 <code>signal 当根及之前数据 + next-bar open + no-overlap + hold 8 bars + 6bps/side</code>，比较 <code>baseline / preflip_reclaim_long_only / preflip_reclaim_symmetric</code> 三臂。</p>
</div>
<div class='card'>
  <h2>主读法</h2>
  <ul>
    <li>baseline mean_total_return = <strong>{pct(base_row['mean_total_return'])}</strong></li>
    <li>preflip_reclaim_long_only mean_total_return = <strong>{pct(primary_row['mean_total_return'])}</strong></li>
    <li>preflip_reclaim_symmetric mean_total_return = <strong>{pct(sym_row['mean_total_return'])}</strong></li>
...
<div class='card'><h2>Setup summary</h2>{render_table(setup_summary, percent_cols={'mean_net_return','total_return','false_follow_through_4bars','left_tail_p5','retention_vs_setup_baseline'})}</div>
...
"""
    write_html(SITE_DIR / "report.html", "Rank110 PSAR preflip reclaim clean replication", report_body)

    reading_body = f"""
<h1>Rank 110 / PSAR pre-flip SAR dot reclaim gate — clean replication note</h1>
<div class='card'>
  <p><strong>一句话：</strong>{escape(verdict_note)}</p>
  <p>这轮把它严格收紧成 <code>PSAR flip 后 N={N_BARS} 根内重穿 pre-flip dot</code> 的最小 honesty test：如果只有 long-only 还能讲得通，而 short 镜像一上就变差，那就只能把它留成 <code>EMA / Fib long-side continuation</code> 的 optional filter note。</p>
  <p><a href='../factors/scout_rank110_psar_preflip_reclaim_15m/report.html'>打开完整 report</a></p>
</div>
"""
    write_html(READING_PATH, "Rank110 PSAR preflip reclaim clean replication", reading_body)
