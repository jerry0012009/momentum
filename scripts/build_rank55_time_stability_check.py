...
def main() -> None:
    ...
    per_asset_bucket = (
        ...
        .agg(
            ...
            false_hold_4bars_rate=('false_hold_4bars', 'mean'),
    ...
    for (setup, variant), group in window_summary.groupby(['setup', 'variant']):
        ...
        verdict = 'watch_only'
        reason = 'mixed'
        if positive_bucket_count == 3 and float(ordered['positive_asset_ratio'].min()) >= (1.0 / 3.0):
            verdict = 'survives_time_check'
            reason = 'all buckets positive but still needs cross-setup honesty'
        elif positive_bucket_count <= 1:
            verdict = 'fails_time_check'
            reason = 'most buckets non-positive'
        elif float(overall_row['positive_asset_ratio']) < (2.0 / 3.0) and float(overall_row['mean_trades']) < 7.0:
            verdict = 'fails_time_check'
            reason = 'only narrow pockets survive and trade count is thin'
        records.append({
            'setup': setup,
            'variant': variant,
    ...
    best_row = stability.sort_values(['verdict', 'overall_mean_total_return'], ascending=[True, False]).iloc[0]
    desk_verdict = 'park / evidence pool'
    desk_reason = (
        '唯一能把三段时间窗口都维持为正的是 ema_psar_long + binary_crash_gate，'
        '但它仍只在单一 archetype 上成立，且每桶平均 trades 仅约 1.7~2.7；'
        '作为共享 crash-risk overlay，这还不足以诚实升格到 P2 / paper candidate。'
    )
...
<head>
  <meta charset=\"utf-8\" />
  <title>Rank 55 · order-imbalance crash-risk overlay time stability check</title>
  <style>
...
<body>
  <h1>Rank 55 · order-imbalance crash-risk overlay（cheap time stability check）</h1>
  <p class=\"muted\">生成时间：{escape(generated_at)}</p>
  <div class=\"callout\">
    <p><strong>问题：</strong>在最小 clean replication 之后，<code>Rank 55</code> 只剩 1 次便宜诚实检查预算。这次不再重跑数据，只用现有 <code>trade_log.csv</code> 把每个 <code>asset × setup × variant</code> 按时间顺序切成 3 个等样本窗口，检查收益是否只是偶然 pocket。</p>
  ...
  <h2>结论</h2>
  <ul>
    <li><code>breakout_short</code> 三个变体在三个时间窗口里全部非正，说明这条 shared crash-risk overlay 没有把 short archetype 稳定救回来。</li>
    <li><code>fib_retest_long</code> 只在零碎窗口小幅转正，幅度基本贴近噪音，不足以支撑升格。</li>
    <li><code>ema_psar_long + binary_crash_gate</code> 虽然是唯一三桶都为正的组合，但它仍只服务单一 archetype，而且每桶平均 trades 偏薄，不能把“局部少亏/小幅转正”误写成 desk 级 shared overlay 通过。</li>
  </ul>

  <h2>时间窗口汇总</h2>
  {render_table(window_summary, percent_cols={'mean_total_return','positive_asset_ratio','mean_false_hold_4bars_rate','min_asset_return','max_asset_return'}, digits_cols={'mean_trades':2})}
  ...
  <h2>最强残留口袋</h2>
  <p>当前样本里最能留下来的是 <code>{escape(str(best_row['setup']))} / {escape(str(best_row['variant']))}</code>：整体回报 <code>{pct(best_row['overall_mean_total_return'])}</code>，正资产占比 <code>{pct(best_row['overall_positive_asset_ratio'])}</code>，每桶最少平均 trades <code>{num(best_row['min_bucket_mean_trades'])}</code>。但它仍不足以把 Rank 55 从 <code>P1 weak candidate</code> 升到 <code>P2</code>。</p>
