...
VARIANTS = ["ema_baseline", "regime_gate_only", "constrained_no_buy", "regime_plus_psar_rsi"]
PRIMARY_VARIANT = "constrained_no_buy"
PARAM_CONFIGS = [
    {"label": "u58_d42_pb45", "up": 58.0, "down": 42.0, "pullback": 45.0},
    {"label": "u60_d40_pb45", "up": 60.0, "down": 40.0, "pullback": 45.0},
    {"label": "u62_d38_pb45", "up": 62.0, "down": 38.0, "pullback": 45.0},
    {"label": "u60_d40_pb47", "up": 60.0, "down": 40.0, "pullback": 47.0},
    {"label": "u60_d40_pb50", "up": 60.0, "down": 40.0, "pullback": 50.0},
]
...
def prepare_bars(asset: str, symbol: str, *, up: float = 60.0, down: float = 40.0, pullback: float = 45.0) -> pd.DataFrame:
    ...
    bars["rsi14"] = rsi(bars["close"], RSI_PERIOD)
    bars["rsi_ema7"] = ema(bars["rsi14"], RSI_EMA_PERIOD)
    bars["psar"] = psar(bars["high"], bars["low"])
    bars["ema_long"] = bars["ema_fast"] > bars["ema_slow"]
    bars["psar_long_ok"] = bars["close"] >= bars["psar"]
    bars["rsi_rebound45"] = (bars["rsi14"].shift(1) < pullback) & (bars["rsi14"] >= pullback)
    regime = np.where(bars["rsi_ema7"] > up, "uptrend", np.where(bars["rsi_ema7"] < down, "downtrend", "fluctuating"))
    bars["regime"] = regime
    bars["variant_ema_baseline"] = bars["ema_long"]
    bars["variant_regime_gate_only"] = bars["ema_long"] & (bars["regime"] == "uptrend")
    bars["variant_constrained_no_buy"] = bars["ema_long"] & (
        (bars["regime"] == "uptrend") |
        ((bars["regime"] == "fluctuating") & bars["psar_long_ok"] & bars["rsi_rebound45"])
    )
    bars["variant_regime_plus_psar_rsi"] = bars["ema_long"] & (bars["regime"] == "uptrend") & bars["psar_long_ok"] & bars["rsi_rebound45"]
    return bars


def build_events(bars: pd.DataFrame, variant: str) -> pd.DataFrame:
    flag_col = f"variant_{variant}"
    active = bars[flag_col].fillna(False)
    entry_transition = active & (~active.shift(1).fillna(False))
    rows = []
    for idx in np.where(entry_transition)[0]:
        row = bars.iloc[idx]
        ...
        rows.append(
            {
                ...
                "rsi_ema7": float(row["rsi_ema7"]),
                "psar_long_ok": int(bool(row["psar_long_ok"])),
                "atr": atr,
            }
        )
        ...
        trades.append(
            {
                ...
                "win": int(net_ret > 0),
                "rsi14": float(event["rsi14"]),
                "rsi_ema7": float(event["rsi_ema7"]),
...
def summarize_trades(trades: pd.DataFrame, nav: pd.DataFrame, candidate_events: pd.DataFrame, asset: str, variant: str, cost: float) -> pd.DataFrame:
    ...
    if trades.empty:
        return pd.DataFrame([
            {"asset": asset, "variant": variant, "cost_bps_per_side": float(cost), "candidate_events": total_events, "trades": 0, "win_rate": np.nan, "avg_net_ret": np.nan, "median_net_ret": np.nan, "total_return": 0.0, "max_drawdown": 0.0, "avg_hold_bars": np.nan, "no_trade_ratio": no_trade_ratio}
        ])
...
def build_regime_summary(trades_df: pd.DataFrame) -> pd.DataFrame:
    if trades_df.empty:
        return pd.DataFrame()
    out = (
        trades_df.groupby(["variant", "regime"], as_index=False)
        .agg(
            trades=("net_ret", "size"),
...
def run_parameter_grid() -> pd.DataFrame:
    ...
    for cfg in PARAM_CONFIGS:
        ...
        for asset, symbol in ASSETS.items():
            bars = prepare_bars(asset, symbol, up=float(cfg['up']), down=float(cfg['down']), pullback=float(cfg['pullback']))
...
def derive_verdict(overall_summary: pd.DataFrame, time_stability: pd.DataFrame, parameter_stability: pd.DataFrame, cross_asset_stability: pd.DataFrame, cost_trade_stability: pd.DataFrame) -> tuple[str, list[str], str, str]:
    ...
    verdict_tag = "park"
    headline = "hard verdict：regime-switch stack 这轮 clean replication 更像 `park / evidence pool`，暂不进入 paper candidate pool。"
    if float(winner_row["mean_total_return"]) > 0 and float(winner_row["positive_asset_ratio"]) >= 2 / 3 and not fail_sets:
        ...
        headline = f"hard verdict：regime-switch stack 的 {winner} 已通过最小 clean replication + Light Stability Pack，可进入 `{verdict_tag}`。"
    bullets = []
    for variant in VARIANTS:
        hit = primary[primary["variant"] == variant]
        if not hit.empty:
            row = hit.iloc[0]
            bullets.append(f"{variant}：mean_total_return {pct(row['mean_total_return'])}，positive_asset_ratio {pct(row['positive_asset_ratio'])}，mean_trades {num(row['mean_trades'],1)}，mean_no_trade_ratio {pct(row['mean_no_trade_ratio'])}。")
    bullets.append("两条轻量诚实守门已通过：规则能明确写成 trade on / trade off；当前实现只用当下 bar 的 EMA / RSI / PSAR / ATR，不用 future label。")
...
def write_report(overall_summary: pd.DataFrame, asset_summary: pd.DataFrame, regime_summary: pd.DataFrame, time_stability: pd.DataFrame, parameter_stability: pd.DataFrame, cross_asset_stability: pd.DataFrame, cost_trade_stability: pd.DataFrame, parameter_grid: pd.DataFrame, meta_df: pd.DataFrame) -> None:
    ...
    summary_table = render_table(
        overall_summary[overall_summary["cost_bps_per_side"] == PRIMARY_COST][["variant", "assets_tested", "positive_assets", "positive_asset_ratio", "mean_total_return", "mean_trades", "mean_no_trade_ratio"]],
        percent_cols={"positive_asset_ratio", "mean_total_return", "mean_no_trade_ratio"},
        digits_cols={"mean_trades": 1},
    )
    asset_table = render_table(
        asset_summary[(asset_summary["cost_bps_per_side"] == PRIMARY_COST) & (asset_summary["variant"] == winner)][["asset", "candidate_events", "trades", "total_return", "win_rate", "no_trade_ratio"]],
        percent_cols={"total_return", "win_rate", "no_trade_ratio"},
        digits_cols={"candidate_events": 0, "trades": 0},
    )
    regime_table = render_table(
        regime_summary[["variant", "regime", "trades", "mean_net_ret", "total_return", "win_rate", "assets"]] if not regime_summary.empty else regime_summary,
...
<head>
  ...
  <title>Scout Seat · regime-switch stack · clean replication</title>
  <style>
...
<body>
  ...
  <h1>Scout Seat · regime-switch indicator stack · 15m crypto clean replication</h1>
  ...
  <div class="card">
    ...
    <ul>
      ...
      <li>变体：<code>ema_baseline / regime_gate_only / constrained_no_buy / regime_plus_psar_rsi</code></li>
      <li>状态：<code>RSI14 的 EMA7 &gt;60 = uptrend；&lt;40 = downtrend；其余 = fluctuating</code></li>
  ...
  <div class="card">
    ...
    {summary_table}
    <p class="muted">artifact：<code>reports/artifacts/scout_regime_switch_stack_15m/overall_summary.csv</code></p>
  </div>

  <div class="card">
    <h2>winner per-asset（6bps/side）</h2>
    {asset_table}
    <p class="muted">这里把 <code>candidate_events</code>、<code>trades</code> 与 <code>no_trade_ratio</code> 一起外显，避免把“少做交易”误写成 alpha 增量。</p>
  </div>
  ...
  <div class="card">
    <h2>parameter neighbor grid（constrained_no_buy）</h2>
    {grid_table}
    <p class="muted">只看 regime 阈值与 pullback 门槛的小邻域，防止把单点阈值 lucky pocket 写成新 alpha。</p>
  </div>
  ...
  <div class="card">
    ...
    <ul>
      ...
      <li>如果 <code>regime_gate_only</code> 已不优于 <code>ema_baseline</code>，就说明“downtrend 禁买”本身没有带来足够增量。</li>
...
def main() -> int:
    ...
    meta_df = pd.DataFrame([
        {"generated_at_utc": datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC"), "candidate_id": "scout_regime_switch_stack_15m_v1", "winner_variant": winner, "verdict_tag": verdict_tag, "verdict": verdict_headline, "sample_window": "Binance 120d / 15m / BTC+ETH+SOL", "next_step": "若为 park，则默认压回 evidence pool 并切到更高边际价值 intake；若为 paper candidate，则下一轮只补最小 candidate-pool writeback。"}
    ])
    ...
    print("[ok] regime-switch stack clean replication generated")
